...
export class ReplayCommand extends BroadcastCommand {
	...
	setPlaySpeed(speed: number, slowMotion: boolean = false): boolean {
		streamDeck.logger.info(`[ReplayCommand] SetPlaySpeed: speed=${speed}, slowMotion=${slowMotion}`);
	...
	setPlayPosition(mode: ReplayPosMode, frameNumber: number): boolean {
		...
		streamDeck.logger.info(`[ReplayCommand] SetPlayPosition: mode=${ReplayPosMode[mode]}, frame=${frameNumber}`);
	...
	search(mode: ReplaySearchMode): boolean {
		streamDeck.logger.info(`[ReplayCommand] Search: mode=${ReplaySearchMode[mode]}`);
	...
	setState(mode: ReplayStateMode): boolean {
		streamDeck.logger.info(`[ReplayCommand] SetState: mode=${ReplayStateMode[mode]}`);
	...
	searchSessionTime(sessionNum: number, sessionTimeMs: number): boolean {
		...
		streamDeck.logger.info(`[ReplayCommand] SearchSessionTime: session=${sessionNum}, timeMs=${sessionTimeMs}`);
