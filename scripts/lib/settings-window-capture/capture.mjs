...
/** Pause after the page loads, before the first capture. */
export const PAGE_SETTLE_MS = 1500;

/**
 * An in-memory {@link SettingsWindowHost} over a plain object.
 *
 * The page writes whole-page snapshots as sdpi-components always does, so the
 * object is mutated rather than replaced, and subscribers are notified — the
 * window's own controls depend on that echo to render what they just saved.
 *
 * @param {Record<string, unknown>} initial - Seed settings (mutated in place).
      ...
      if (result?.value !== true) {
        throw new Error(
          `The settings page has no nav button for "${tab.pane}". ` +
            `Rebuild the Stream Deck plugin, or update SETTINGS_WINDOW_TABS if the tab was renamed.`,
        );
      }
