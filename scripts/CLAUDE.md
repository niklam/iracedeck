...
- `lint-pack.mjs` — `pnpm lint:pack <packDir>` (the folder holding `voice-pack.json` and `voice/`): says loudly what the plugin would only skip quietly — grammar problems in a voice's `callouts.json`, every compile skip the pack did not mean with its reason, frames and fragments that fail, clips shipped that nothing references, references to clips not shipped, and a per-voice `<voice>: N of M callouts scripted` summary. Exit 0 clean, 1 on any problem, 2 when it could not run (usage, a path that is not a directory, a missing dist — `pnpm build` first, as for the generator). The rules are `lintPack` in `packages/audio-scenarios/src/reference/lint-pack.ts`; `lib/lint-pack-run.mjs` is the argument handling and the `node:fs` port (`runLintPack(argv, io) → exit code` — injected `io`, a code returned rather than `process.exit` called, the same testable shape as the `lib/` helpers below) and owns `PLUGIN_PLAYED_GROUPS` (`names`, `toggle` — the clip groups plugin code plays by path with the active voice, outside any script; each entry names the file that plays it, and a new by-path consumer is a new entry), handed to the pure linter as `pluginPlayedGroups` so a pack shipping them is never told they never play. A lint of the bundled `packages/audio-assets` tree is still not expected to be clean (the bundled voice's exceptions live in its coverage test, not in the linter, and it ships a few genuinely unreferenced groups).

Both read the catalog through `lib/catalog-engine.mjs`: `registerCatalogEngine()` registers the real `registerPitCrew` off the built dist against a stub bus/audio/logger **once per process** and hands every later caller the same engine (the radar / spotter / pit-speeding sub-engines are module singletons — a second registration throws); `setActiveVoice(voice)` is process-global and the only thing that changes afterwards.
