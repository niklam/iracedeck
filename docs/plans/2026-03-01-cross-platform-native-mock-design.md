...
`@iracedeck/iracing-native` is a C++ N-API addon that wraps the iRacing SDK (Windows-only). Every downstream package (`iracing-sdk`, `stream-deck-plugin`) depends on it, and all have `"os": ["win32"]` in their `package.json`. This means `pnpm install`, `pnpm build`, and `pnpm test` all fail on macOS.
...
| `packages/stream-deck-plugin/src/shared/keysender.d.ts` | Type shim for keysender |
