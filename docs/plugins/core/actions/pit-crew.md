...
After that intro — in qualifying and race sessions — the engineer adds a **setup warning** when the loaded setup's *name* looks wrong for the session: in qualifying on a race-looking name ("Our setup name suggests that we're on a race setup. Please double-check."), or in a race on a qualifying-looking name ("Our setup name suggests that we're on qualifying setup. Please double-check."). It's a heuristic on `DriverInfo.DriverSetupName` only — it never changes anything, just asks you to verify. The match is two user-editable, case-insensitive regular expressions (one per session kind) under **Setup Warning Patterns**, each with a **Reset to default**; an invalid pattern is flagged in the PI banner and the warning is skipped until you fix it. Toggle the whole feature in **Race Engineer Callouts → Setup Warning**.
