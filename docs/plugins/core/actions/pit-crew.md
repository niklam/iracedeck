...
Once the car is in the box the engineer reads every **pit-service status** transition iRacing reports (`PlayerCarPitSvStatus`, issue #479): service in progress, service complete, the four positioning errors (too far left / right / forward / back), a bad parking angle, and "can't fix that" — the only iRacing-exposed signal that a queued damage repair won't actually be performed. All eight share `family: "pit-status"`, so wiggling from *too far left* to *too far right* preempts the stale line cleanly; closing transitions back to the idle state are silent. Each status has its own opt-in under **Race Engineer Callouts → Pit Service Status**, all on by default.

The five positioning errors additionally **repeat while they stay uncorrected** (issue #951). iRacing announces a positioning error once and then leaves the status latched, so a driver who backs up but stops short of the box is left unserved in silence. The translator therefore re-emits a dedicated `pitService.positioningRepeat { status }` every `PIT_STATUS_REPEAT_INTERVAL_MS` (2 s) while the error persists — with **no cooldown** — and five terse nag scenarios ("Still too far forward.", "Back it up.") speak it. Four gates shape it:

- **Held while the car is moving.** Any `Speed` sample above `PIT_STATUS_MOVEMENT_SPEED_MPS` (0.05 m/s, ~2 in/s — box corrections happen at a crawl) marks the car moving immediately, and it counts as at rest again only after `PIT_STATUS_REST_SETTLE_MS` (500 ms) continuously below it. The debounce is deliberately asymmetric: noise costs at most a delayed nag, while the engineer never talks over a driver mid-correction.
- **Armed from the LEVEL, not the transition edge.** A transition arms the clock for the normal case, but `pitStatusInitialized` resets on a plugin restart (#870), an SDK reconnect, an `IsOnTrack: false` blip, and the replay-flip `wipeStateForReplay` — none of which leave an edge to arm on. A latched error with no armed clock therefore starts one from the level. Edge-arming alone reproduced the very silence this issue fixes.
- **Bounded to pit road** (`OnPitRoad !== false`, missing data counts as on pit road), so a status still latched after the driver drives out can't nag forever wherever the car next stops. Deliberately not `PlayerCarInPitStall` — an overshot car may read `false` there, which is the repro case.
- **Re-checked at speak time.** `queueable: false` does not drop a fire that outranks a *lower*-weight in-flight line — the engine queues it, and a pending fire replays without re-running `where:`. The sequence is wrapped in an `if:` (the #669 furled precedent) re-reading the live `PlayerCarPitSvStatus`, so a nag queued behind the readback can't tell an already-parked driver to back up. Unknown telemetry plays, keeping the harness firable.

A status transition restarts the cycle and speaks the new error's full conversational call. The nags carry their own `family: "pit-status-repeat"` and an explicit `PIT_STATUS_REPEAT_WEIGHT` (40, between `CHATTER` and `NORMAL`): same-family preemption ignores weight, so sharing the transition family would let a nag chop the initial call mid-sentence — instead a nag arriving mid-call is simply dropped and retries on the next cadence tick, while a fresh error outranks a playing nag and speaks in full right after it. Delivery is terse (no radio-beep frame, the pit-box count-in precedent) and the repeats ride their sibling's existing opt-in — no new setting.
