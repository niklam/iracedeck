...
- Manual vs autofuel mode is derived from `dpFuelAutoFillActive`; autofuel availability from `dpFuelAutoFillEnabled`. In **Add Amount** and **autofuel** modes the displayed add comes from `PitSvFuel` (the live pit request iRacing reports), so the readout matches the in-sim black box rather than the optimistically-dialed value (#726); in autofuel it is open-loop — the lap margin is not in telemetry.
