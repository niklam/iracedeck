...
- **Rotate**: Adjusts the dialed value by the step size per detent, in the displayed unit. The dial spans the full tank range (`0` to capacity) in both modes:
  ...
  - **Fill To** — the dialed value is the desired **total** after the stop, kept a whole integer in the displayed unit. The amount to add (`target − current`, plus a fixed **+1 L** safety buffer so you finish at least the target) is sent to iRacing (`pit.fuel`) and **auto-recomputes** as fuel burns. The recompute and re-send only happen **while fueling is on** — when fueling is off the fuel amount is **not** updated (rotating still plans the new target; turning fueling on, or pressing, sends it). The readout shows `→ <target>` (e.g. `→ 65 L`).
...
- **Fill To** - The dial sets the total you want in the tank after the stop, as a whole integer in the displayed unit. The amount to add (target − current, plus a fixed +1 L safety buffer so you reach at least the target despite fuel burned during the top-up) is kept topped up automatically (re-sent every 30 s) as fuel burns **while fuel-fill is on**; while fuel-fill is off the amount is **not** updated (rotating still plans the new target, and turning fueling on or pressing sends it). A red vertical target line marks the target on the bar
...
- In Fill To mode the request is recomputed and re-sent every 30 s **while fuel-fill is on**, so the target stays topped up as fuel burns; while fuel-fill is off the fuel amount is **not** updated (the user's toggle-off is respected — rotating still plans the target, and turning fueling on or pressing sends it). The Fill To add includes a fixed +1 L safety buffer so you reach at least the target despite fuel burned between the live top-up recompute and the actual pit stop (the buffer is clamped to the tank, so it is dropped when the target is the full tank).
