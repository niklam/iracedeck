...
All planned actions will be built into the existing `@iracedeck/iracing-plugin-stream-deck` package (`com.iracedeck.sd.core`). The plugin supports up to 32 main actions, each containing multiple sub-actions selectable via Property Inspector dropdown.
