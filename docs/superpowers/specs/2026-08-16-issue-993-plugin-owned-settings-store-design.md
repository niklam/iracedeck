...
#### Amendment (phase 2, 2026-08-16)

Phase 2 shipped 4.4 and 4.5 with the deliberate departures below; the code (`pi-components/src/settings-channel/`, `pi-components/src/pi-settings-bridge/`, `pi-components/src/ulanzi-bridge/`, `deck-core/src/settings-window-guard.ts`) is the reference.

- **The guard is token-first, not Origin-first.** §4.3 and §9 assumed the window's `Origin`-then-token/cookie order would serve PIs too. It cannot: a PI is a `file://` page (`Origin: null`) or a host-served origin, so the strict Origin check rejects every one of them. `authorizeSettingsRequest` therefore authorizes a request carrying the valid launch token **regardless of `Origin`** — the token is the secret (per-launch, 48 hex chars, reachable only through the window's URL and the plugin's own store) — and only a token-less request falls through to the original rules, where `Origin` must match the loopback origin exactly before the `SameSite=Strict` cookie is even compared. The cookie path therefore keeps its DNS-rebinding mitigation intact, and no CORS header is ever emitted, so a hostile page cannot read what it manages to send. The server reports every upgrade verdict + `Origin` through `onUpgradeDecision`; the controller logs it at debug.
- **On Ulanzi the router sits in `UlanziBridgeSocket`, not in `translate.ts`.** §4.5 put the reroute inside the translator; in the code the translation layer is untouched — `UlanziBridgeSocket` runs the same router as the Elgato/Mirabox bridge, and only frames the router chooses to send to the host still pass through `elgatoToUlanzi`/`ulanziToElgato`. Its bootstrap read is therefore the plugin-scoped Ulanzi `getGlobalSettings`, and one bundle still ships (`ulanzi-pi-bridge.js` grew the behaviour).
- **One shared router, bootstrapping proactively.** Both bridges run the same pure state machine (`settings-channel/router.ts`: `idle → bootstrapping → connecting → loopback`, plus `fallback`) rather than two implementations, and it does **not** wait for sdpi's first `getGlobalSettings` as §4.4 described: it sends its own bootstrap read the moment the host socket opens (after sdpi's register frame), so the channel is usually up before sdpi asks anything. Frames sdpi sends while bootstrapping/connecting are queued and replayed at whichever surface wins.
- **Settle timer per phase; fallback and late switch.** `BOOTSTRAP_TIMEOUT_MS` (3 s) is armed on entering `bootstrapping` **and on every `connect()`** — the first attempt, a late switch, a reconnect — because a loopback transport that never calls `onOpen`/`onClose` would otherwise leave sdpi's `getGlobalSettings()` promise (and every `global` control) pending forever. A missing channel, a refused/stalled connect, or a silent host falls back to the host path with a `console.warn`, never a blank PI. From `fallback` the router still switches **late** when a host push carries a channel it has not already tried — this covers PIs opened in the seconds before the plugin's mirror write. Retry semantics distinguish the two closes: a channel that was _refused_ (closed before it ever opened) is remembered and not retried; one that opened and later closed is forgotten, so a push repeating it reconnects. A host close before the bootstrap completed returns the router to `idle` (a retried connection can bootstrap again); from any later state it lands in `fallback`.
- **The guarded mirror IS the bootstrap write.** The phase-1 Amendment's `{ ...getGlobalSettings(), _settingsChannel }`, skipped when `getSettingsStoreSource() === "fresh"`, is exactly what phase 2 sends — `hostMirrorPayload(channel)` in `global-settings.ts`, called from each plugin's store-ready block right after the channel is published into the store. There is no second host write.
- **The build asserts the bridge layout.** §7's "exactly one bridge script before sdpi" check is a build step, not a test: `assertBridgeInjectionPlugin` (`pi-components/src/build/inject-bridge-plugin.mjs`) runs in `closeBundle` on all three plugins and throws — naming the file — unless every generated page that loads sdpi carries its expected bridge exactly once, immediately before `<script src="sdpi-components.js">`, with no other known bridge present. Elgato/Mirabox expect `pi-settings-bridge.js` on action PIs, Ulanzi `ulanzi-pi-bridge.js`, and all three `settings-window-bridge.js` on `settings-window.html`.
- **Still open (§9, unchanged in substance):** whether UlanziStudio answers the plugin-scoped `getGlobalSettings` read _within a PI's session_. It does not answer the plugin's own connect-time read (hence the adapter's boot bootstrap), and no hardware is available here — a community tester has to settle it. Until then the Ulanzi PI's global settings may simply take the fallback path and behave exactly as they did before phase 2; everything that goes through the plugin (the window, persistence) is unaffected.
