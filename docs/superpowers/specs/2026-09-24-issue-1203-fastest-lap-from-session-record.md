...
### Amendments after the code review

The branch's full review found that, once a walk writes its landing into a record later presses trust without walking, every way a walk can land wrong becomes a permanent wrong jump. So the walk records only what it has verified, and never acts on a map or sample it has not:

- **Recording needs verified convergence**: the sample before the back-step must show the target car on the lap before, at distance ≥ 0.999, in the target `(SessionNum, SessionUniqueID)`; lap-step exhaustion, a search that did not move or settle, or a landing elsewhere logs `walk did not converge` and records nothing (the replay still ends playing).
- **The session map is keyed by `SubSessionID` and the uid set**, is dropped on SDK disconnect, keys its entries by `(sessionNum, uid)` so a restarted session is its own entry, and reads the last session's end live from `ReplayFrameNum + ReplayFrameNumEnd` on every walk instead of freezing it at build time. `SessionUniqueID` is a small per-run counter, so a second `.rpy` in the same plugin process reused the first one's ids.
- **No walk and no jump from the car**: replay commands only work out of the car, so a press while `IsReplayPlaying` is false sends nothing, including no camera switch. A search that times out aborts the map build and caches nothing.
- **One cursor owner, process-wide** (`iracing-actions/src/shared/replay-cursor.ts`): the walk claims the replay cursor, and any other jump — Replay Control's other modes and Replay Markers' Next / Previous — cancels the claim before sending its command. A second Jump to Fastest Lap press is checked before it sends anything.
- **A search's decision sample is taken after the minimum gap**, not at the first two-read hold, because the sim may still be resolving the lap boundary during it.
- **A walked lap joins the live session record** when no record matches its `(sessionNum, uid)` pair but exactly one session has the `sessionNum` and the frame lies within its recorded span, and a lookup falls back past a sparse pair match to the one other session with the car and lap. A restart's laps lie past the first instance's span (frames only grow), so the two instances of the Homestead case stay separate.
