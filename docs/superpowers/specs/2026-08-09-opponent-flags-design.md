...
- **Weights:** raise trigger `WEIGHT.NORMAL`; range-entry trigger `WEIGHT.SAFETY` (approaching an impaired car is the safety case). Both `queueable: true`, `interrupt: false`, **family-less** (the #622/#923 rationale: lines describe different cars and sustained states — queue, never chop, never drop).
- **Scenario split:** two scenarios per flag (raised / entered-range) so the triggers carry different weights while mapping to the same `OpponentFlagCalloutId` (the #772 two-stage precedent), plus one aggregate — 9 scenarios.
...
- **Per-car cooldown:** 30 s (`OPPONENT_FLAG_CAR_COOLDOWN_MS`).
...
- **9 scenarios:** `pit-crew.opponent-flag-<flag>-raised` and `pit-crew.opponent-flag-<flag>-entered-range` for each of furled/black/meatball/disqualify, plus `pit-crew.opponent-flag-others`. Scenario ids use the spoken subject names (`meatball`, not `repair`). All family-less, `queueable: true`, `interrupt: false`; raised at `WEIGHT.NORMAL`, entered-range at `WEIGHT.SAFETY`, aggregate at `WEIGHT.NORMAL`.
