...
- *"A preserved copy is useless without a retry loop."* Wrong, but less wrong than the first draft of this line claimed. Preserved settings are recoverable — by deleting the settings file, which re-runs the migration from scratch, and that is now the documented rescue. What that draft ALSO said, and what the second max review disproved, is that the install recovers on its own: it does not. The ceiling writes the durable marker and clears the countdown in the same `becomeReady` call, so the persisted file carries no countdown, every later start takes the `unanswered === 0` fast path, and the host is never asked again. **Giving up IS a one-way door**, and the `source === "host"` retirement path written to open it was unreachable in production — a test asserting otherwise had seeded both markers at once, a combination nothing can write. The dead path was deleted rather than left looking like a safety net, and automatic recovery is now [#1047](https://github.com/niklam/iracedeck/issues/1047): re-ask once when the running plugin version differs from the one that gave up, since "never asked under this build" is exactly the abandoned cohort's state.

One argument survives review and is now the load-bearing one, because it is what makes suppression free rather than a trade: **the marker is only ever set when the host answered nothing, and a host that answers nothing does not answer a Property Inspector's bootstrap read either** — both are the same `getGlobalSettings` with a non-empty `actionid`. So the `_settingsChannel` the mirror would carry is unreachable by the only party that wants it, and the "PIs channel-less forever" cost the original ceiling comment warned about is not a real cost in the state the marker describes.

That premise now has evidence under it rather than reasoning. A read-only probe against a live UlanziStudio measured two things. A populated bucket's reply carries **no `code` field**, so the client's `#868` ack drop-filter keeps it — which retires the worry that an empty bucket's reply might be silently dropped while a Property Inspector's is kept, the one asymmetry that could have made the gate cost something. And a **foreign** uuid, one the host has never stored anything under, gets **no reply at all** — so whichever way an empty bucket behaves, the two readers behave alike: answered together, or silent together. The probe also narrowed a claim this repo had been over-reading: #1039's "uuid changes neither the answer nor the bucket" was measured only across iRaceDeck's own uuids, which the host normalises to one plugin. What could not be measured is a *registered* plugin with an empty bucket — the fresh-install case — because the dev machine has one plugin installed, leaving "silent because unknown" and "silent because empty" indistinguishable. All three facts, including that boundary, are recorded in `.claude/rules/global-settings.md` and `packages/deck-adapter-ulanzi/CLAUDE.md`.
...
- **Users exactly at the ceiling are preserved, not restored.** With `_migrationAbandoned` in place their still-intact host copy stops being overwritten, so their settings survive in UlanziStudio. Nothing reads them back on its own: the marker is permanent, and the rescue is deleting the settings file, which re-runs the migration from scratch — documented in the changelog and on the Settings page. Automatic recovery is [#1047](https://github.com/niklam/iracedeck/issues/1047). This is the honest limit of the change, and the point at which it stops being a fix and starts being a containment.
- **A downgrade is not a rescue, and the spec must not sell it as one.** An older build has no `_migrationAbandoned` check, so it loads such a file, sources it `"file"`, passes every gate it has, and mirrors defaults over the preserved copy on its first start — destroying it through the route this change exists to close. That is acceptable only because a downgrade to a build with the unaddressed read is already the broken state; it is not a recovery path and is not offered as one.
...
**Test 1 — is the migration read answered with no deck attached?** This is the issue itself. Setup: pre-3.0 settings present in the host store, `%LOCALAPPDATA%\iRaceDeck\Settings\Ulanzi\global-settings.json` deleted, **no Ulanzi deck attached** — that last condition is the whole point, since with a deck attached the old `willAppear` re-drive masks the result.

**Order matters, and getting it wrong produces a silent no-op that looks like a pass.** Stop UlanziStudio, confirm the process is actually gone, *then* delete the file, *then* start it again. Deleting while the host is still running does not clear anything: the plugin writes the file straight back on its way out — `process.on("exit", () => settingsStore.flushSync())`, plus whatever the 250 ms debounce still holds — so the next start finds a file, logs `Global settings loaded from the settings file`, and never enters the migration path at all. The first attempt at this test did exactly that. Nothing in the log announces the mistake; you simply do not see the migration lines you were looking for, which is easy to misread as a failure and just as easy to misread as a pass.

Start UlanziStudio and read the plugin log in order. Predicted, verbatim:
