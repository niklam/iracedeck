...
**Drop a data-less global-settings ack rather than translating it.** Added during review. UlanziStudio confirms a write with an ack-shaped `didReceiveGlobalSettings` (`code` set, no settings), and the bridge translated any such frame into an empty payload for sdpi. That is the #1039 symptom by another route: mid-bootstrap the router reads empty settings as "no `_settingsChannel`" and falls back permanently, and on the fallback path it blanks every global control already on screen. The plugin's own socket has dropped exactly these frames since #868 (`handleFrame`), so the bridge takes that rule rather than inventing one. Reachable only now that the read is answered at all, which is why it surfaced with the fix rather than before it.

The bridge's version is deliberately **narrower** than the plugin socket's, and review is what forced the distinction. The plugin drops on an *empty payload*; the bridge drops only on an *absent settings field*. An explicit `settings: {}` is a real answer — it is what a read against an empty store returns, and on a first install it is the only answer there is. Dropping it would leave sdpi's `getGlobalSettings()` promise pending forever and render exactly the blank PI this change exists to remove, so the guard would have reintroduced the bug in the one case where the store has nothing to show. The residual risk runs the other way: if this host ever confirms a write with `settings: {}` rather than a bare ack, that frame is passed through and blanks the controls on the fallback path. That trade is deliberate — the empty-store case is reachable by every new install, the write-ack-with-payload case is speculative, and the loopback path drops host pushes outright, so it can only bite a PI that already failed to bootstrap.
