...
| One lap to green | "One lap to green. Take the inside line behind car oh nine." |
...
- **The position moved to its own call, at about 35% into the one-to-green lap** — the normal position wording ("We're currently P fourteen"), reading your **race position from the canonical order** — see the 2026-09-19 correction below for why it is not the pace-row order that was first chosen. A new event, `caution.lastLapCheckpoint`, marks the moment: the player's lap distance rises through 35% for the first time after `caution.oneLapToGreen`, while still under caution. The whole call sits in one `optional` clause, and here that is correct rather than the empty-sequence defect: a position call with no position has nothing true to say.
- **"One to go" becomes "One lap to green"**, pairing with "Two to green". (The maintainer chose "One lap to green" over the "One to green" first written here, when he re-recorded the lines.)
...
- `caution.fieldCaught` keeps its name, since the signal (the pace car has picked the field up) is unchanged and only what the engineer says about it moved. Its `restartPosition` payload goes, because nothing reads it any more. `caution.lastLapCheckpoint` carries no payload either: its call reads the race position live at speak time, and a published field nothing reads is a contract that has to be maintained for no one.


**Corrected 2026-09-19, from a maintainer test** (`master/local/telemetry-snapshot-20260919-094304-382.json`). The position call said **"P21"** while his display correctly showed **P19**, in a 20-car field. Reproduced from the snapshot, it had two separate causes:

- **The double-file position formula assumed the two lanes pair off evenly.** At that one to green, line 0 held 11 cars (rows 1..11) and line 1 held 9 (rows 0..8): a lapped car and the waved-around player had gone to the tail of one lane. `line 0, row R → 2R − 1` gave the player, at line 0 row 11, position 21, which no 20-car field can have. The lineup is now counted as one combined order: every lined-up car keyed `(R, 0)` on line 0 and `(R + 1, 1)` on line 1, sorted, and counted. That reproduces `2R − 1` and `2R + 2` exactly when the lanes are balanced, and stays correct when they are not. The restart position still feeds `caution.isLeader` and the payloads, so this fix matters beyond the position call.
- **The pace-row order was the wrong source for a spoken race position.** Car #7 was two laps down and lined up *ahead* of the player but *behind* him in the race, so even a correct restart order says 20, while the race position, the thing "P19" means, is 19. The pace-row order was chosen on the argument that the running order lags early in a caution (pace row 14 against official 11 in the first capture). That argument does not reach this call: it fires a lap or more after the pickup, once that lag has gone, and lapped cars are exactly where the pace row misleads. So the position call reads the **canonical live race position** (`getLivePosition`), which is what `@.claude/rules/race-positions.md` requires for any race position anyway. **The pace-row carve-out now covers who you follow and which lane, not a spoken position.**

The same snapshots are the first capture in which `CarIdxPaceFlags` is set: the player carries `WavedAround`. Every earlier capture had it at 0 for every car.
