...
**It carried a validation risk, and that risk is now closed by observation rather than by data.** None of the 52 captured telemetry snapshots in `local/` shows `Voltage` below 12 V — the observed set is exactly `{12, 13.37, 13.4, 13.7, 13.8, 14.2, 14.4, 14.8}` — including the three captures where the engine is confirmed stalled, which read 14.2 / 14.2 / 13.4 V. Those captures were all taken in-car or in replay and never caught an ignition-off moment, so nothing in this repository could evidence the rule. The maintainer confirmed the behaviour as domain authority up front, and manual testing on real hardware then confirmed it in the only way available: both keys were exercised in the sim and reported working. Had `Voltage` not moved with the ignition off, the Ignition key would have sat permanently green and been reported broken rather than working.

Be precise about what backs the threshold, because an auditor reading only the capture set would reasonably conclude it was guessed: **the evidence is a hardware observation, not a captured frame.** If a future change makes the ignition state suspect, capturing a snapshot with the ignition off is the first thing to do — it is the one piece of evidence this design has never had.
