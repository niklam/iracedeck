...
4. **Follow-up derives the tag from GitHub state using the repo's own conventions.** `.claude/rules/issue-workflow.md` sets the milestone and the assignee when implementation *starts*, so "assigned or milestoned" is the repo's definition of in progress; the issue text also named "or a PR open", and that clause is dropped here as redundant with the convention that produces it. Released is computed from git, not guessed, and from every trace a closed issue leaves rather than one: the merge commits of its closing PRs (`closedByPullRequestsReferences`, returned by the list call), the commit named on its timeline's closed event, and every commit whose *subject* carries `(#N)` and is not a `docs(specs)` commit — this repo's squash titles carry the issue number (`build-and-commit.md`), which is what catches the 53 of 293 completed issues that have no linked PR because they closed through a release-branch PR or a back-merge. The two exclusions are load-bearing: a commit whose *body* cites `(#N)` is usually about something else, and the spec commits carry the number too but land on `master` before the feature ships, so counting them reported `Released (v3.1.0)` for four unreleased features when the rule was first tried. The union of the stable tags (`vX.Y.Z`, no pre-release suffix, judged by `semver`, already a root devDependency, which is all a repo script needs) containing any of those commits yields the lowest version, which is the one that shipped. A closed issue with no commit found anywhere stays `Completed` with a note that Released must be set by hand. **Closed as duplicate** is its own case (this repo uses that state reason): the tool sets no tag for it and reports the row, since the honest reply points the requester at the canonical issue, which only the maintainer can name.
...
| --- | --- | --- |
| `list [--untagged]` | Every post in the channel: id, title, created, archived flag, reply count, applied tag names, post link. `--untagged` keeps only posts with no status tag. (No author handle here: the thread listing carries only an owner id, and resolving it would cost a request per post; `show` has the handle from the starter message for free.) | `GET /guilds/{guild}/threads/active` filtered on `parent_id`; `GET /channels/{channel}/threads/archived/public` paged on `before` = last `archive_timestamp` while `has_more` (a listing that cannot be paged to its end throws rather than returning a silently short list); `GET /channels/{channel}` once for `available_tags` |
| `show <post-id>` | The starter message and every reply with author and timestamp, the reaction breakdown and total, applied tags, post link, and the ready-made source line. A post whose starter message was deleted reports its author as unknown and its votes as zero rather than promoting the oldest reply. | `GET /channels/{post}/messages?limit=100`, paged on `after` if a post exceeds 100 |
...
- `readConfig(root, env)` — the three variables, with a precise error naming the missing one. It reads a *copy* of the process environment, so the token loaded from the file never lands in `process.env`; the entry additionally strips the variable from the environment it hands `gh` and `git`.
...
- `expectedTag(issue, releasedVersion)` — the follow-up table below as a function of `state`, `stateReason`, `assignees`, `milestone`, and a release version or `null`; `null` for a duplicate-closed issue, meaning no tag.
- `rank(tag)` and `shouldPropose(current, expected)`.
- `postLink(guild, postId)` / `parseSourceLine(text)` — the latter finds the `Requested on Discord:` line in an issue body, anchored to that prefix so a Discord link quoted elsewhere in the body can never be mistaken for the source; follow-up also checks its guild id against the configured one.
- `sourceLine({ link, handle, votes })` — the exact line format. `show --json` emits it as `sourceLine`, and the skill copies that string when filing and when adopting rather than retyping the format, so the writer and the reader (`parseSourceLine`) cannot drift.
...
2. For each issue derive the expected tag:
   ...
   | Closed as duplicate | none — reported, never proposed; the maintainer points the post at the canonical issue |

   Released needs `git fetch --tags` first, then the closing commits from the three sources decision 4 names (`gh pr view <n> --json mergeCommit` for each closing PR, the timeline's closed event, `git log --all --fixed-strings --grep="(#N)"` filtered to subjects that carry `(#N)` and are not `docs(specs)`) and `git tag --contains <sha>` for each, taking the lowest stable version over the union. A closed issue with no commit anywhere stays `Completed` and carries the note that Released must be set by hand. A failing `gh` or `git` call marks that one row with a note and leaves every other row intact; it never aborts the run.
3. `list --json` for the current tags. Propose a change only where `shouldPropose(current, expected)` holds, and never for a post on the standing list (decision 8), for an issue with no source line, for a post the channel no longer has, or for a duplicate-closed issue; every other note (a failed release lookup, no commit found) accompanies a normal proposal so the maintainer sees both. Present the whole batch — post, current tag, expected tag, reply text — for approval, then run `reply` and `tag` per post.
