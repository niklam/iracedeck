...
**Named pools are an alias facility, not a catalogue** (decided 2026-09-05, replacing the first draft's "named pools stay the recommended form"). Every callout is a pool — the set of takes of one line, file-additive — so a name that merely restates the path (`flag-blue → flags/blue`, thirty times over for the flags) is boilerplate. A pool gets a name only when the name carries a decision: it points at a different group than its name suggests, or a second line must not share a no-repeat tracker with the first (`pit-action-acknowledgment` is the case in the catalog), and then the reason goes in its `comment`. Otherwise a script addresses the clips directly with `pool:<group>/<base>`, which resolves through the same manifest-derived members and the same tracker. The layout-protection argument for names was real but weak: a pack that reorganises `flags/` edits its own scripts, and ours never reorganise a shipped group. #1065 applies the same test per family as it migrates.
