...
- **Stage 3 — the 3.3.0 line, designed 2026-09-07 (see *Stage 3 — dropping the bundle* below).** Drop `default` from the distributable. This is Release N+1. It was recorded here as "the only thing that changes is the bundled list", and that turned out to be false for a reason the section below records.
...
## Stage 3 — dropping the bundle (settled 2026-09-06 and 2026-09-07)

The four rulings of 2026-09-06 are recorded in the sections above: the launch-time fetch is unconditional, it runs outside the settings-store-ready block, a seeded `default` is refreshed, and the Settings card is the only progress surface. This section records the implementation design settled on 2026-09-07 and the finding that forced it.

**The "one-word edit" was wrong.** `voice-packs.mjs` promised that flipping `default` to `bundled: false` changes nothing else. It does: `generate-audio-manifest.mjs` filters the committed `manifest.json` to `BUNDLED_VOICE_IDS`, so the flip empties that manifest of `default` — and twenty-odd test files, the scenario harness, the pack-reference generator and the audio-assets coverage test all read it as "the authored voice". The harness copies its clips and loads its scripts through the same bundled filter, so it would boot with no voice at all. The distinction the code was missing is **authored versus bundled**: what the repo holds, and what a plugin ships.

**Two manifests, one generator.** `manifest.json` goes back to describing the whole authored tree — every voice under `voice/` — which is what every consumer except the plugins wants. A second generated, committed and freshness-tested `manifest.bundled.json` is that manifest filtered to `BUNDLED_VOICE_IDS`; the three plugins import it and nothing else does. `processAndCopyAudioAssets` and the harness's script loader take an explicit `voices: "bundled" | "all"` choice, defaulting to bundled; the harness passes `all`, because it auditions what is authored, not what is distributed. After this, `bundled: false` really is the one-line flip, and the scanner's `reservedVoices` list becomes empty on its own.

**The launch step lives in deck-core, once.** `voice-pack-launch.ts` exports `ENSURED_VOICE_PACK_ID` (`"default"`) and a `createVoicePackLaunchStep` that runs, in order: the installer's sweep, its seed (kept — the bundled-seed rule is permanent and it is a no-op with nothing bundled), the **ensure**, and a status republish. All three plugins call it; the three duplicated startup blocks go, which is part of what #1126 asks for.

**Ensure means "make `default` match the catalog, and bring every other installed catalog pack up to date".** For `default` the step calls the installer, whose existing compare already does the work: absent installs, a provenance digest that differs from the catalog updates, a matching digest downloads nothing. For every other installed pack whose provenance names the catalog as its source and whose digest is behind the catalog's, the step calls the same install — an update the user would otherwise have to notice and press for. **Settled 2026-09-07 (Niklas): updates are automatic, and there is no carve-out for a hand-edited or sideloaded `default`.** A `default` folder with no provenance record, or with a foreign one, is replaced like a stale one — the old tree goes to the trash, as every update does. The default voice working matters more than letting a user customise iRaceDeck's own pack in place; anyone who wants to is told to copy the folder to a different name, where the empty-folder seed rule and the missing provenance leave it alone. The website says so in as many words. A sideloaded pack under any other name is never touched: the automatic update reaches only packs the catalog installed. This is what #1105 asked for, so that issue closes with this stage.

**`default` is un-removable.** Today the card shows it as "Included with the plugin" with no Remove button because the bundle provides it. Once nothing is bundled it would become an ordinary installed pack — and a Remove followed by the next launch's ensure would reinstall the 8 MB the user just deleted, the plugin arguing with them. So the card keeps `default` without a Remove button, with the row reading that iRaceDeck keeps this one current. Rejected: letting it be removed and having the ensure honour a tombstone — a new marker for a case the un-removable row makes impossible.

**Ordering: after the settings load settles, not gated on it.** The step is not conditioned on `isSettingsStoreReady()`; it waits for the load to *settle* — ready, or the fail-closed give-up — through a `whenSettingsStoreSettled()` promise added to `global-settings.ts`. The reason is the `_devBaseUrl` catalog override, which the catalog client reads from the settings cache: fired before the load, the launch fetch would silently ignore it. On the unreadable-settings path the step still runs, about thirty seconds late, and the voice still arrives — which is the whole point of ruling 2.

**Retry, and retry harder while the Race Engineer is on — settled 2026-09-07.** A failed ensure is retried on a timer, whether `default` is missing, `default` is stale, or another catalog pack is behind: a failed update leaves a voice whose newer callouts are silent, so it is not the benign case the earlier table made it. The schedule is read live from `pitCrewRaceEngineerEnabled`: while it is on, the retry runs at 1, 2, 5 and 10 minutes and then every 15; while it is off, at 5, 15 and 60 minutes and then hourly. Flipping the gate on, and the Rescan button, both retry at once. The timer stops the moment every pack the step is responsible for is at the catalog's digest, and an Install pressed in the window joins the same in-flight operation as ever. Failures the installer classes as permanent for this catalog answer — a hash mismatch, a malformed archive, a catalog that omits or cannot offer `default` — are given up rather than retried on the tight schedule, and **re-observed hourly** (amended after the branch review, 2026-09-07: the earlier wording, "until the catalog answer changes", named the failure TTL as its mechanism, but that TTL governs a failed fetch, not a successful fetch followed by a permanent install failure — so nothing would ever have re-asked, and a captive portal or a momentarily incomplete catalog would have cost the whole session). Every retry is silent and opens no window; the structural test extends to the new module.

**Two more rulings from the branch review (2026-09-07).** A `default` whose provenance record says it matches the catalog but which the scanner cannot use — its clips gone, the record intact, as an antivirus quarantine or an interrupted sync leaves it — is **force-reinstalled**: `install(id, { force })` skips the digest short-circuit, because the digest is a claim about what was written, not what is on disk now; with `default` un-removable this was the one state with no in-app escape. And the scanner **claims voices for the managed pack first**, then alphabetically (`priorityPacks`): with nothing bundled there is no reserved voice id any more, and a sideloaded folder sorting before `default` could otherwise have taken the `default` voice from the pack the launch step keeps installing.

**What does not change.** The pack version stays 1.0.0 in this stage: no clip has changed since the 3.2.0 tag, so a bump would be a lie, and the refresh path is exercised the day a clip change bumps it (#1131 is the candidate, and belongs before the 3.3.0 tag for that reason). The catalog `isEnabled` predicate stays a constant true, per ruling 1. The seed's empty-folder rule stands for every other pack.
...
| Situation | Behaviour |
|---|---|
| Offline on first run | No voice; the Settings window's Race Engineer card says so, with Retry; the launch step retries on its own schedule (see *Stage 3*) and again next start. Every non-audio feature unaffected. |
...
- **All three plugins** — packs root wired into `initializeAudio`; scan, seed, first-run install and refresh in the startup sequence; audio copy step filtered to the bundled set. Since stage 3: one deck-core launch step in place of three startup blocks, and `manifest.bundled.json` as the compiled-in manifest.
...
1. **Is `default` also published to the catalog? — settled 2026-09-02: yes.** It is listed like any other pack rather than special-cased. That is what keeps `default` out of every code path as a special case, and it is why a new install after the bundle is dropped has something to fetch at all. (It did not make stage 3 a one-line change — see *Stage 3 — dropping the bundle*.)
