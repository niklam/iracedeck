...
`onThreshold` **returns a boolean** — whether it actually drew anything. That is what lets the helper own "did we show something", so a surface whose gesture has no knowable outcome arms, shows nothing, and pushes no pointless revert frame at release; without it every surface would carry the same guard. The flag gating is a constant-folded ternary at construction (`__FEATURE_DIAL_FEEDBACK__ ? createHoldPreview(…) : NOOP`) rather than a condition at each of the four call sites, so the call sites stay unconditional.

This section first claimed that gating also kept the preview out of the non-Elgato bundles. **Checking the built `plugin.js` disproved it**: the preview's strings and pure resolvers are in the Mirabox and Ulanzi bundles, because they are reachable from class methods terser cannot prove unused, and the pending bar is reachable from the strip renderers on every platform. The gate is a runtime one — no helper constructed, no timer armed, no frame pushed, all asserted by the surfaces' flag-off tests — and the dead weight is a few hundred bytes against Mirabox's 20 MB ceiling, so it was not worth restructuring the surfaces to chase. Recorded because "the flag tree-shakes it" is the kind of claim that gets repeated until someone relies on it.
