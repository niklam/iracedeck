...
### 5. The two unit vars stay, pointed at unit-only clips, and the reference voice does not use them

**Amended 2026-09-22 by the maintainer.** `sessionStart.degreesUnit` and `raceStart.degreesUnit` stay registered, so a pack author who wants the unit spoken can still script it. The reference voice records none of their clips, and both of its scripts drop the third step from their temperature clauses.

The vars now resolve to unit-only clips, `session-start/unit-celsius` and `session-start/unit-fahrenheit` ("Celsius," / "Fahrenheit,"), chosen by the snapshot's `tempUnit`. The value clip now ends in "degrees,", so the old `degrees-*` names would compose "twenty eight degrees, degrees Celsius,". A pack that keeps the unit records the two new clips.

This design was first written the other way: delete both vars, and reject keeping them as resolvers that always return `null`. That objection was to a var that can never produce a clip, which would be a trap in the published reference. These vars produce a real clip wherever a pack has recorded it, so the objection no longer applies. Keeping them also shrinks the pack-format break. The compiler catches an unknown var per **entry**, so deleting the vars would have cost a pack that still scripts `{{sessionStart.degreesUnit}}` its **whole** session-start brief: greeting, session line, pit-speed limit and wetness. Now such a pack loses only what it has not recorded. Its temperature figure stays silent until it records `numbers-degrees`, and its unit word stays silent until it records the `unit-*` clips.
