/**
 * Unit tests for the pit-road speeding diff (issue #912).
 *
 * The two properties worth pinning hardest, because both are decisions that a
...
import { diffPitSpeeding, PIT_SPEEDING_HYSTERESIS_MPS } from "./pit-speeding.js";
import type { PendingEvent } from "./types.js";

/** 80 kph, the shape `resolvePitSpeedLimit` produces from session YAML. */
const LIMIT = 80 / 3.6;
...
describe("diffPitSpeeding", () => {
  ...
  describe("end edge — speed", () => {
    it("ends once the speed falls a full hysteresis band below the limit", () => {
      const state = createInitialState();
      const { events, emit } = collect();
    ...
    it("holds the episode inside the hysteresis band so a speed sitting on the limit cannot flutter", () => {
      const state = createInitialState();
      const { events, emit } = collect();
  ...
  describe("end edge — every other exit", () => {
    it.each([
    ...
    it("ends the episode when Speed goes missing", () => {
      const state = createInitialState();
      const { events, emit } = collect();
      ...
      // be asserted without a speed. Failing to silence.
      expect(names(events)).toEqual(["pitSpeeding.started", "pitSpeeding.ended"]);
    });
