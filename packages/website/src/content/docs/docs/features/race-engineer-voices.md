...
**Every callout comes from the script.** Flags, pit service and the readback, the start lights, position and gap readouts, lap times, incidents, the spotter — each one is a line in the voice's `callouts.json`, so a pack decides the wording of all of them, and the radio frame around them, from the same file. A voice with no script therefore has nothing to say: it loads, it can be selected, and it stays quiet, with the banner below telling you why.
