...
In this release the **flag callouts** are the first to come from the script. The remaining callouts still use the wording that ships with iRaceDeck — spoken in whichever voice you pick, and wrapped in that voice's radio frame, which the script also defines — and move into the script in a later release, at which point a pack decides all of them. A voice with no script plays those remaining callouts without a radio frame, since there is no script to define one.
