...
Focus iRacing Window is a **global setting**: one switch that applies to all of iRaceDeck, not per button. You'll find it in the [Settings window](/docs/getting-started/settings/), on the **General** tab, as **Focus iRacing window before sending keys**.
...
To switch it on, open the [Settings window](/docs/getting-started/settings/) and tick **Focus iRacing window before sending keys** on the **General** tab. The change takes effect immediately — no restart needed.
