...
In race sessions, `position` and `class_position` follow the **live race order** — they update continuously as cars move (the same live ordering the [Session Info](/docs/actions/display-session/session-info/) Position readout uses while racing on track), rather than only refreshing at the start/finish line. A car physically in the pits shows its **live track position**, not its held official position. Before the green flag the whole field shows its **starting grid position** instead, so `race_ahead` and `race_behind` resolve to the cars you line up between — including drivers still sitting in the garage. The value renders blank only for a car that is in neither order: not on the grid before the start, or not yet classified into the running order after it.
...
| `{{telemetry.ReplayPlaySpeed}}` | Replay playback speed, as iRacing reports it: in slow motion a value of N plays at 1/(N+1)x, so `4` means 1/5x |
