...
![The Race Engineer tab, showing the live toggles, startup policies, voice, the Radio Frame beeps and ambience switches, the voice packs you have installed and the ones available to download, and callout opt-ins](../../../../assets/settings-window/race-engineer.png)

Everything about the Race Engineer and the radar: the live on/off toggles and their startup policies, the voice, your name, the output device and the three volumes with **Test** buttons beside them, the two halves of the radio frame, the voice packs you have installed and the ones available to download, the setup-warning patterns, and the per-callout opt-ins.

**Race Engineer Voice** and **Your Name** choose who talks to you and what they call you. **Output Device** sends the engineer to a specific device — useful if you want him in your headset while the sim runs on speakers. The three volumes are independent: **Race Engineer** is the voice, **Background** the radio beeps and the pit ambience around it, **Radar** the proximity ticks.

**Radio Frame** is the pair of checkboxes under the Background slider. Every spoken line is framed by the voice's own radio frame — the one its callout script defines. In **Default**, iRaceDeck's own voice, that is a short beep as the channel opens, the pit-lane atmosphere running underneath while the engineer talks, and a beep as it closes; a voice pack may frame its calls its own way. A voice with no callout script says nothing at all, so there is no frame to hear either. **Radio beeps** and **Pit ambience** switch those two halves separately — turn one off and only that half goes; the words are untouched, and the other half keeps playing. Both are on by default, and **Background** still sets the level of whichever is on. The frame only ever wraps speech: a callout that has nothing to say in the voice you are using gets no beeps either, so you never hear a pair of clicks around silence. Callouts that already play without the frame — the terse pit-box nudges, the spotter, the corner names — are unaffected by either switch.

**Installed Voices** lives in the **Voice Packs** card next to the Race Engineer card (below it when the window is narrow), above **Available to Download**. It lists the voice packs iRaceDeck has loaded, each with its version, where it came from, and the voice or voices it provides — so two packs with the same name that provide different voices are easy to tell apart, and each voice is named as it is in the **Race Engineer Voice** dropdown. Anything it could not load is listed underneath with the reason why — so a pack you dropped into the folder that is not showing up in the dropdown tells you what is wrong with it. **Rescan voices** below it picks up a pack you have just added, without restarting anything. See [Race Engineer Voices](/docs/features/race-engineer-voices/) for where packs live, how to install one, and what a pack's callout script decides.

A voice that is listed but has no callout script — a pack built before scripts existed, or one whose script is missing — is still selectable, but every callout is skipped in it — every callout comes from the script — so the engineer goes quiet. While such a voice is the one selected, a banner at the top of this window and of every key's Property Inspector names it, so a quiet engineer is never a mystery: reinstall the pack, or pick another voice. The banner is re-checked after every rescan and every change of voice, and disappears by itself once the selected voice has a script.
