...
When one car of a pair is stopped or crawling — you've spun, or the car ahead has wrecked — the gap switches to a live estimate of how long the chasing car needs to cover the distance at its current pace, so a stopped driver sees the pursuit counting down instead of a frozen number, and a wreck ahead counts down as you approach it.
