...
Go over the pit lane speed limit and a short tick starts repeating, about once a second, for as long as you're over it — a beeper, like a road car's over-speed chime. It stops the instant your speed drops back under. Unlike everything else the engineer says this isn't a spoken line and it doesn't wait its turn on the radio: it plays straight away, because a warning that arrives after the penalty is no warning at all.
