...
The engineer also stays quiet when refueling wouldn't help: in a lap-limited race no count is announced once the estimate covers the laps left in the race, nothing fires on your own final lap (in timed races too, based on your crossing under the white flag), and the family is silent after the checkered. Earlier in the race this only suppresses on a positive "the fuel covers the race" determination — if the lap count can't be read, the callouts keep coming — and it is re-evaluated every lap, so a sudden jump in consumption that shrinks the estimate below the race distance still gets called. The final lap and everything after the checkered are always quiet, though, even when the estimate says the tank is short: at that point the estimate isn't precise enough to tell a real shortage from a scare, so the engineer leaves the last lap to you.
