...
- **White** — a two-stage final-lap alert in races: a heads-up when the white flag comes out (the leader has started the final lap — *"White flag. We're about to start the final lap."*), then *"This is the last lap."* as you cross start/finish and begin yours. When you're the one leading, the heads-up is skipped and the last-lap call lands right at the line. Practice and qualifying keep their single raise-time line. Both stages share the White callout toggle.
...
- **Checkered** — session-aware: practice, qualifying, and race finishes get distinct lines. Spoken as you take the flag at the start/finish line, not the moment the session ends — so on your final qualifying lap the call comes when you actually cross the line. (In practice and qualifying, if you're in the pits or out of the car when the checkered flies, it's spoken right away; driving into the pits after the flag also counts as done. In a race you take the flag at the line — even from pit lane.)
