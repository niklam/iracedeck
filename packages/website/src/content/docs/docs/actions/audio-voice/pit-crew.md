...
- **Meatball** — the only flag callout marked **urgent + preempt**: it cancels in-flight engineer chatter mid-message, since failing to pit on a meatball costs a black-flag penalty. All non-meatball flag callouts share a `flag` family so a newer flag preempts an older one. That family is *not* what keeps a race restart clean, though this page used to claim it was: **Yellow cleared** is queueable, so instead of being dropped for the green it waited for the radio to go quiet and then played after it — the "yellow's clear" + "green flag" double-talk exactly as described. What actually silences it is upstream of the family — a caution that went full-course now reports no cleared yellow at all, so there is nothing left to queue (see **Yellow cleared** above).
