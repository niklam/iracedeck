...
Fuel Dial turns a Stream Deck+ dial into a fuel controller — it is a dial-only action (for fuel control on regular buttons, use [Fuel Service](/docs/actions/pit-service/fuel-service/)). A bare turn adjusts fuel — and when autofuel is engaged in iRacing, it adjusts the autofuel lap margin instead. Five configurable gesture slots (Push, Long Press, Push + Turn, Tap Display, Long Touch) each run a fuel action, you watch the live readout with a continuous two-segment fuel bar on the touch strip, and the dial works on Mirabox and Ulanzi knobs too.
