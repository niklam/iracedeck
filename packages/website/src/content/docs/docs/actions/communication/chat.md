...
Sending a message opens the chat window, pastes the text, presses Enter, then closes the chat window. Three waits in that pipeline are configurable in the [Settings window](/docs/features/settings-window/#delays), on the **Delays** tab under **Chat** (all default 200 ms):
