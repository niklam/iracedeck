...
- **Enable dual-press** (default *on*) — when off, the key stays a pure read-only display and presses do nothing. When on, presses dispatch to the matching adjustment binding (e.g. View Brake Bias dispatches to *Brake Bias +* / *Brake Bias −*), so configure those bindings in the Property Inspector's **Key Bindings** section, or on the Settings window's **Key Bindings** tab under *Setup Brakes*.
...
Adjustment modes with a live value (on Setup Brakes: ABS Adjust, Brake Bias, Brake Bias Fine, Peak Brake Bias, Brake Misc, and Engine Braking) can render as **paired keys**: place two keys with opposite directions next to each other (or three, with a View key in the middle) and both show the live value — no separate display key needed. Choose the look under **Key Style**:
