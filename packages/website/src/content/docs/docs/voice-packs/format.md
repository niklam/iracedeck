...
| `{ "var": "lapTime.second" }`                                                 | Play what the variable resolves to — a clip, or a pool it picks from. Nothing aborts the callout unless the step sits inside an `optional`.                                                |
