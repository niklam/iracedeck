...
A voice's `id` only has to be unique within its own pack; a second voice declaring the same `id` in the same pack is ignored, with the reason listed. Outside the pack folder iRaceDeck refers to a voice as `<pack id>::<voice id>` — the voice above is `my-pack::my-voice` — so two packs may each ship a voice called `matt`, and both are installed, listed and played from their own clips. You never write that form yourself: `voice-pack.json`, the clip paths and `callouts.json` all use the bare voice `id`, and the full form turns up only in the plugin's log and in the scenario harness. Because the two ids are joined with `::`, neither may contain it. The kebab-case rule already rules that out, but an `id` that tries is refused with the separator named as the reason, so a pack that qualified an id by hand is told why rather than only that the id is malformed.
...
A voice with no `callouts.json` still loads: it is a clips-only voice, and every callout is skipped in it. When the file is there, it is read as UTF-8 — a leading byte-order mark, which some Windows editors write, is tolerated — and refused before it is parsed if it is larger than 1 MB, many times the size of the script of the reference voice (`default`, the one iRaceDeck publishes). A voice whose script exists but cannot be read, is too large, or fails the rules below is left out of the pack, with the reason listed; the pack's other voices still load.
...
| `{ "connector": true }`                                                       | Play one take of the pool named `connector` — a filler word between two phrases. The script must define that alias under `pools`; the reference voice's script uses no connectors.                  |
...
Define a name only when it carries a decision — an alias onto a different group than the name suggests, or a second line that must not share a no-repeat tracker with the first — and say why in the comment. Otherwise address the line directly as `pool:<group>/<base>`; the reference voice's script defines no pools at all.
...
- An entry that names something the engine does not know — an unknown variable, condition, case, case key, pool alias, fragment or frame — is skipped, with one warning in the log naming the reference: `Voice "my-pack::my-voice": scenario "pit-crew.flag-green" skipped — unknown var "lapTime.minutes"`. The callout is silent in that voice until the entry is fixed.
...
- Expansion is budgeted: an entry or frame whose included fragments unfold to more than 2,000 steps is refused with `fragment expansion exceeds 2000 steps`. The largest body in the reference voice's script is about thirty.
- With debug logging on, the log also carries one line per voice — `Voice "my-pack::my-voice": 3 of 149 callouts scripted` — and the list of ids that are not.
