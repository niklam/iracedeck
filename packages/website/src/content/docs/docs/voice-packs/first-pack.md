...
The wording is yours. The path is what matters: `voice/<voice-id>/<group>/<base>-NN.mp3`, one folder per group, lowercase `.mp3`. A clip one folder too shallow, or exported as `.MP3`, is refused rather than installed silent. Every take of a line is one **pool** — record a second take later as `green-02.mp3` and the engine alternates between them; nothing in the script changes.
...
It prints the problems it finds grouped by voice, and a summary line per voice: `my-voice: 3 of 149 callouts scripted`. It exits non-zero on any problem. With skip-by-default, quiet failure is the design, so this is the one place a pack is told anything loudly; run it every time you add lines.
