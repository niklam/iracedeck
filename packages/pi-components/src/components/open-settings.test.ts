...
import "./open-settings.js";

describe("ird-open-settings", () => {
  ...
  function mount(attrs: Record<string, string> = {}): HTMLElement {
    const el = document.createElement("ird-open-settings");
  ...
  it("sends the openSettings command to the plugin on click", () => {
    ...
    expect(send).toHaveBeenCalledWith("sendToPlugin", { event: "openSettings" });
  });
