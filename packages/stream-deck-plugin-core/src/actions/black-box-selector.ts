...
/**
 * Black box display name mapping
 */
const BLACK_BOX_NAMES: Record<string, string> = {
 ...
 */
function getModeContent(mode: string, color: string): string {
  if (mode === "next") {
    // Up/Next arrow
    return `    <path d="M36 20 l-8 10 h5 v8 h6 v-8 h5 z" fill="${color}" stroke="none"/>`;
  } else if (mode === "previous") {
    // Down/Previous arrow
    return `    <path d="M36 38 l-8 -10 h5 v-8 h6 v8 h5 z" fill="${color}" stroke="none"/>`;
  } else {
    // Direct mode: display lines
    return `    <line x1="18" y1="20" x2="54" y2="20" stroke="${color}" stroke-width="1.5" stroke-linecap="round"/>
    <line x1="18" y1="26" x2="48" y2="26" stroke="${color}" stroke-width="1.5" stroke-linecap="round"/>
    <line x1="18" y1="32" x2="52" y2="32" stroke="${color}" stroke-width="1.5" stroke-linecap="round"/>`;
  }
}
...
function generateBlackBoxSelectorSvg(settings: BlackBoxSelectorSettings): string {
  ...
  if (mode === "next") {
    ...
    labelText = BLACK_BOX_NAMES[blackBox] || blackBox;
  }
