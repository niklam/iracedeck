...
- `references.ts` — `collectLiteralClips(steps)` → every literal clip a step list plays (bare path strings and `{ clip }` objects, through every branch, includes not followed), the one walk the completeness and coverage checks share so a new step form cannot leave a copy behind; `collectStepReferences(steps)` → `StepReferences` (`pools`, `vars`, `conds`, `cases`, `includes` of ONE step list, an include not followed) for a consumer that attributes references to the entry or fragment making them — the generated pack-author reference lists each callout's own (#1066); and `collectScriptReferences(script)` → `ScriptReferences` (`StepReferences` merged over every sequence the script plays, plus): `scenarioIds`, `pools`, `vars`, `conds`, `cases` (with the keys each maps), `includes`, `frames`, `fragments` — the names the script DEFINES, so a consumer can state the include rule as `includes ⊆ fragments` (#1065) — and `unincludedFragments`: the defined fragments nothing LIVE includes (not a non-skipped entry, not a frame, not another fragment reached from one of those, transitively), which the engine's compiler checks on their own and a consumer counting references against clips leaves out. Every list deduped and sorted; walks `then` / `else` / `optional` / every `of` branch, the frames' own `open` / `close`, and every fragment's `sequence` (a pool used only inside a fragment is still a reference). A `skip: true` entry contributes its id and nothing else — the compiler never reads past the skip, so a `frame` or a `sequence` left beside it is not a reference.
...
**Evolving the grammar.** `fragments` was added under `schema: 1` without a version bump, which is fine only because no plugin that reads `schema: 1` has shipped without it (3.2.0 is unreleased). Once a grammar version has shipped, an additive key needs either a `CALLOUT_SCRIPT_SCHEMA_VERSION` bump — so an older plugin reports "written for a newer version of iRaceDeck" instead of `<key>: unrecognized key` and dropping the voice — or a `minPluginVersion` on the pack's catalog entry, so the older plugin never offers the pack at all.
