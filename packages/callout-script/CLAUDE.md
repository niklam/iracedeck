...
- `references.ts` — `collectScriptReferences(script)` → `ScriptReferences`: `scenarioIds`, `pools`, `vars`, `conds`, `cases` (with the keys each maps), `includes`, `frames`, `fragments` — the names the script DEFINES, so a consumer can state the include rule as `includes ⊆ fragments` (#1065) — and `unincludedFragments`: the defined fragments nothing LIVE includes (not a non-skipped entry, not a frame, not another fragment reached from one of those, transitively), which the engine's compiler checks on their own and a consumer counting references against clips leaves out. Every list deduped and sorted; walks `then` / `else` / `optional` / every `of` branch, the frames' own `open` / `close`, and every fragment's `sequence` (a pool used only inside a fragment is still a reference). A `skip: true` entry contributes its id and nothing else — the compiler never reads past the skip, so a `frame` or a `sequence` left beside it is not a reference.
