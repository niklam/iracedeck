...
export interface PITemplatePluginOptions {
  /**
   * Directory containing .ejs template files (source)
   * Relative to the package root
   */
  templatesDir: string;

  /**
   * Directory to output compiled .html files
   * Relative to the package root
   */
  outputDir: string;

  /**
   * Directory containing shared partials
   * Can be absolute or relative to package root
   */
  partialsDir: string;

  /**
   * Additional directories to search for partials (optional)
   * Searched in order after partialsDir
   */
  additionalPartialsDirs?: string[];
}
 ...
 */
function loadDataFiles(dataDir: string): Record<string, unknown> {
 ...
 */
function createTemplateRequire(templateDir: string): (modulePath: string) => unknown {
 ...
 */
export function piTemplatePlugin(options: PITemplatePluginOptions): Plugin {
