...
- `global-settings.ts` — Plugin-level global settings manager. Single writer (#993): the plugin-owned `SettingsStore` file is truth, the deck host is read once for migration and ignored after that; `isSettingsStoreReady()` reports when the cache reflects the store
- `settings-store.ts` — The plugin-owned settings file (#993): `SettingsStore`, `createFileSettingsStore` (atomic temp+rename write, 250 ms trailing debounce, `flush()`, corrupt file moved aside as `global-settings.corrupt-<iso>.json`), `createMemorySettingsStore` (tests/harness), `resolveSettingsStorePath` (`%LOCALAPPDATA%\iRaceDeck\Settings\<ecosystem>\global-settings.json`, `IRACEDECK_SETTINGS_PATH` override), `settingsStoreFolderName`
- `open-folder.ts` — `openFolderInExplorer(path)` + `explorerSelectArgs` (#993): reveals the settings file with `explorer.exe /select,<path>`, detached. The second `child_process` use beside the settings-window launcher; the path always comes from the plugin's own store, never from a page, and it deliberately does not go through `openUrl` (http(s)-only on every host)
