...
- `global-settings.ts` — Plugin-level global settings manager. Single writer (#993): the plugin-owned `SettingsStore` file is truth, the deck host is read once for migration and never read again; `isSettingsStoreReady()` reports when the cache reflects the store and `getSettingsStoreSource()` how it was filled (`"file" | "host" | "fresh"`). `hostMirrorPayload({ port, token })` builds the ONE host write per start the PI bridge bootstraps from — the full cache plus `_settingsChannel`, `undefined` (skip the write) before the store is ready or when it started fresh, since a host `setGlobalSettings` replaces the whole object (a fresh start persists the `_migrationPending` marker, `MIGRATION_PENDING_KEY`, so later starts ask the host again — up to `MIGRATION_RETRY_STARTS` unanswered starts, then the file is accepted — and never mirror a defaults file over an unread host copy); an unreadable file is retried with a doubling back-off (`LOAD_ATTEMPTS`, `LOAD_RETRY_DELAY_MS`)
...
- `mouse-pointer-service.ts` — Mouse pointer singleton (#926): `initMousePointer`, `movePointerToSim`, `PointerMoveResult`, `DEFAULT_POINTER_X_FRACTION` / `DEFAULT_POINTER_Y_FRACTION`, `_resetMousePointer`. Deliberately a SIBLING of the focus service rather than part of it — one owns the foreground, the other owns where the pointer goes, and neither needs the other. Same injected-delegate shape and the same hand-kept native mirror, guarded by `pointer-move-result.test.ts`. Composing the two (focus, then move) is feature policy and lives in `@iracedeck/iracing-actions`' `shared/mouse-to-sim.ts`.
