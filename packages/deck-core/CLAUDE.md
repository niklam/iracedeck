...
- `global-settings.ts` — Plugin-level global settings manager. Single writer (#993): the plugin-owned `SettingsStore` file is truth, the deck host is read once for migration and neither read nor written after that (phase 2 adds one guarded-mirror bootstrap write for the PI bridge); `isSettingsStoreReady()` reports when the cache reflects the store
