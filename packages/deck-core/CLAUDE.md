...
- `settings-store.ts` — The plugin-owned settings file (#993): `SettingsStore`, `createFileSettingsStore` (atomic + debounced, corrupt file moved aside), `createMemorySettingsStore` (tests/harness), `resolveSettingsStorePath`, `settingsStoreFolderName`
