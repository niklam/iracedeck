...
- `global-settings.ts` — Plugin-level global settings manager. Single writer (#993): the plugin-owned `SettingsStore` file is truth, the deck host is read once for migration and never read again; `isSettingsStoreReady()` reports when the cache reflects the store and `getSettingsStoreSource()` how it was filled (`"file" | "host" | "fresh"`). `hostMirrorPayload({ port, token })` builds the ONE host write per start the PI bridge bootstraps from — the full cache plus `_settingsChannel`, `undefined` (skip the write) before the store is ready or when it started fresh, since a host `setGlobalSettings` replaces the whole object
- `settings-store.ts` — The plugin-owned settings file (#993): `SettingsStore`, `createFileSettingsStore` (atomic temp+rename write, 250 ms trailing debounce, `flush()` plus a synchronous `flushSync()` each plugin wires to `process.on("exit")`, corrupt file moved aside as `global-settings.corrupt-<iso>.json`), `createMemorySettingsStore` (tests/harness), `resolveSettingsStorePath` (`%LOCALAPPDATA%\iRaceDeck\Settings\<ecosystem>\global-settings.json`, `IRACEDECK_SETTINGS_PATH` override), `settingsStoreFolderName`
