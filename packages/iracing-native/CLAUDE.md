...
**Why the target is a parameter and not a native constant.** The placement policy belongs to the caller, exactly like the caller-supplied `sendChatMessage` delays and `sendScanKeySequence`'s `holdMs`. It keeps iRacing UI knowledge out of the addon — and it paid off in #1029, which made the target **user-configurable** (an anchor per axis plus a percentage offset) with no native rebuild: the fractions are now resolved by `deck-core`'s pure `sim-pointer-target.ts` from four `mouseToSim*` global settings. `deck-core`'s `DEFAULT_POINTER_X_FRACTION` / `DEFAULT_POINTER_Y_FRACTION` remain the no-argument fallback, and the shipped defaults still resolve to exactly those (horizontally centered, one eighth down from the top, on iRacing's own top-of-screen UI band).
