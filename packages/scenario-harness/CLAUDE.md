...
The radio frame's two opt-outs, `raceEngineerRadioBeeps` and `raceEngineerPitAmbience`, are seeded on and read live at frame expansion through `getFrameOptions` in `src/main.ts` — `POST /api/settings {"patch": {"raceEngineerRadioBeeps": false}}` is heard on the next callout. The UI's settings grid does not offer them yet; the endpoint does.
