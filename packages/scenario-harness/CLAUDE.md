...
`flag-caution-restart` ("Flags → Caution → restart (green)") is the reference and the only one today: `Caution` up, held; then the caution bits down and `Green` up on the same tick; then flags cleared. Two things every sequence owes — it **ends by restoring a sane `SessionFlags`**, or the harness is left stuck under a flag and the button cannot be pressed again, and its `description` says the mock SDK must be **CONNECTED**, because a disconnected translator sees no ticks and the button is then silent for the wrong reason. Hold times come off the translator's own constants where one exists (`YELLOW_CLEARED_HOLD_MS`), so the wait stays provably past the window being listened through.
