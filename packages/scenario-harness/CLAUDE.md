...
`flag-caution-restart` ("Flags → Caution → restart") is the reference and the only one today. It replays the `SessionFlags` of a caution and restart captured at an oval (`local/telemetry-watch-20260917-191825-092.jsonl`), in order and with only the holds compressed to about 27 s: `CautionWaving`; static `Caution`; `+OneLapToGreen`; `+GreenHeld`; the restart, `Green | Servicible | StartGo`, with every caution bit dropping on that same tick; `StartHidden` in place of `StartGo`; then no flag shown (`Servicible | StartHidden`). With the race session and hot-lap presets applied it plays the caution-waving, full-course yellow and green-held lines and "Go, go, go!" — no green-flag line, because `StartGo` suppresses it exactly as at a race start — and then nothing. `scenario-shortcuts.test.ts` pins the step values to the capture and the translator's events to that list. Two things every sequence owes — it **ends by restoring a sane `SessionFlags`**, or the harness is left stuck under a flag and the button cannot be pressed again, and its `description` names the presets its promised lines need and says the mock SDK must be **CONNECTED**, because a disconnected translator sees no ticks and the button is then silent for the wrong reason. Hold times come off the translator's own constants where one exists (`YELLOW_CLEARED_HOLD_MS`), so the wait stays provably past the window being listened through.
