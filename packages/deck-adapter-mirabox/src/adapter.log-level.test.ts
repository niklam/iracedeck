...
// level that loggers created via createLogger read on every call, so flipping
// it affects already-created loggers without recreating them.
describe("VSDPlatformAdapter log level", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
