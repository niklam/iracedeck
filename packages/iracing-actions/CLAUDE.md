...
- `dial-box.ts` — shared Stream Deck+ dial "dash box" renderer + `dialAppearanceFields` settings fragment: the seven Setup dial surfaces resolve their per-setting accent plus user color overrides (`resolveDialBoxColors`) and route rendering through `renderDialBox` (#811)
