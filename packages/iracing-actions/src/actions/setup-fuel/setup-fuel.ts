 ...
 */
export function parseSetupFuelSettings(raw: unknown): SetupFuelSettings {
  const parsed = SetupFuelSettings.safeParse(raw);
...
export class SetupFuel extends ConnectionStateAwareAction<SetupFuelSettings> {
  ...
  override async onKeyDown(ev: IDeckKeyDownEvent<SetupFuelSettings>): Promise<void> {
    ...
    if (settings.keyStyle !== "legacy" && hasPairedValueSource(settings.setting)) {
      ...
      this.repeat.onKeyDown(ev.action.id, {
        ...
        execute: async () => {
          await this.executeSetting(setting as SetupFuelAdjustSetting, direction);
  ...
  private parseSettings(settings: unknown): SetupFuelSettings {
    return parseSetupFuelSettings(settings);
  }
