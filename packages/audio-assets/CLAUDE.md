...
`manifest.json` is the **runtime manifest**, and since #1034 stage 3 it has a sibling: `generate:manifest` writes both, from one generator. `manifest.json` names every **authored** voice — every `voice/<id>/` directory — while `manifest.bundled.json` names only the **bundled** slice (`BUNDLED_VOICE_IDS`), which is what a plugin distributable actually contains and what the scanner's reserved voice ids derive from. They are identical until a published voice stops being bundled. Since #1034 it is only the **built-in half** of what the plugin plays: installed voice packs under `%LOCALAPPDATA%\iRaceDeck\Race Engineer\Voices` contribute their own clips at runtime, each pack acting as its own audio root, and the scenario engine consumes the union (`mergeManifests` in `@iracedeck/audio-scenarios`). So a voice missing from this file is not necessarily missing at runtime. It lists clips only — never a voice's `callouts.json`, which ships beside the clips but is read by the voice-pack service, not resolved as a clip. Rebuild after adding/removing clips with:
