...
Whichever of the two a plugin carries, it is only the **built-in half** of what that plugin plays: installed voice packs under `%LOCALAPPDATA%\iRaceDeck\Race Engineer\Voices` contribute their own clips at runtime, each pack acting as its own audio root, and the scenario engine consumes the union (`mergeManifests` in `@iracedeck/audio-scenarios`). So a voice missing from these files is not necessarily missing at runtime. They list clips only — never a voice's `callouts.json`, which ships beside the clips but is read by the voice-pack service, not resolved as a clip. Rebuild after adding/removing clips with:
