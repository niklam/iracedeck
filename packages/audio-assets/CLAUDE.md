...
## The callout script — `scenarios`, `frames`, `pools`, `fragments` → `voice/<voice-id>/callouts.json` (#1064, #1065)

The same `configs/<voice-id>.voice.json` also carries the voice's **callout script**, under four optional top-level keys beside `groups`: `scenarios` (scenario id → what to say when the code-declared contract fires — `sequence`, or `skip: true`, plus the `comment` and `test` lines the published reference is built from), `frames` (named open/close wrappers, `radio` by default; `none` is reserved and can never be defined), `pools` (pool name → `{ group, base }` — an alias facility, usually absent: a sequence addresses its clips directly as `pool:<group>/<base>`, and a name is given only where it carries a decision), and `fragments` (#1065: name → `{ comment, sequence }`, a sub-sequence several entries of the SAME script include as `"@<name>"` — the compiler inlines it, so it is sharing between scenarios, never an escape hatch for complexity inside one; the bundled voice has two, `readback-body` and `gap-readout`). One file per voice, on purpose: it is what a pack author is handed. The grammar — types, Zod schema, `parseCalloutScript` — is `@iracedeck/callout-script`; the how-to and the vocabulary a script can reference live in `.claude/rules/race-engineer-callouts.md`.

That file never ships (the ElevenLabs voice id, the TTS settings and every line of text stay here), so the four maps are **extracted** into a committed artifact inside the voice tree:

```bash
pnpm generate:callout-scripts     # every configs/*.voice.json → voice/<voice-id>/callouts.json
```

The artifact is the four maps under a `schema: 1` header, keys in the author's order (it is the reading order of the reference), two-space LF JSON. A voice that authors none of the keys still gets one, with empty maps — a valid, clips-only voice whose callouts are all skipped. `scripts/generate-callout-scripts.mjs` loads the configs through `loadVoiceConfigs`, so a malformed key fails there naming the file and the path (`Invalid default.voice.json:` + one `<path>: <message>` line per issue), re-checks each extracted script with `parseCalloutScript`, and writes nothing until every voice has passed. The freshness test `src/callout-scripts.test.ts` fails when any artifact drifts from its config, naming the command — same pattern as `manifest.json` and `changelog.json`. In a Claude Code session the post-edit hook runs it whenever a config is saved; run it by hand outside one, or when the hook reports a failure — and commit the artifact with the config either way.
...
Pools are derived per-voice from the clips that actually exist (a pool is every `voice/<voice>/<group>/<base>-NN.mp3` plus the bare `<base>.mp3`, resolved by the `@iracedeck/audio-scenarios` engine at fire time), so voices do **not** need identical clip sets: a voice may record fewer or more `-NN` variants of a line, or omit a callout entirely — that voice simply doesn't play it. Missing keys are allowed; wording can vary per voice (different engineer personalities).

What IS checked is that each voice's own script covers its own clips — `src/generate/script-coverage.test.ts`, which replaced `voice-parity.test.ts` in #1065. The old test refused a non-default voice any `<group>/<base>` that `default.voice.json` lacked, on the grounds that a base the canonical voice does not know is referenced by no pool; that held only while every pool came from a code registry, and once a voice ships its own script its own base is referenced by its own sequence — not dead, just not ours. The replacement is per voice and stronger. For every `configs/<voice-id>.voice.json`: (a) **orphans** — every `<group>/<base>` authored in a group the script addresses must be referenced by the script (a step, a frame, a fragment or a `pools` alias); an unreferenced base is a misspelled take or a line that lost its consumer, and it is what the old test could never see in `default.voice.json` itself; (b) **dangling** — everything the script references, in either spelling, must be authored, and every plain pool name it draws from (a `{ connector: true }` step included) must be defined under `pools`, since a reference to nothing is a fire-time abort (#835); (c) a vacuity floor for the bundled voice. A group NO step addresses (`position-number`, `names`, the lap-time digits, …) is var-driven — a code resolver returns `poolRef(group, base)` at fire time — and is skipped whole, because the vocabulary lives in `@iracedeck/audio-scenarios`, which depends on this package. A group a script addresses for some bases while a resolver produces the rest (`session-start`, `incidents`, `pit-limiter`) is declared in the test by shape, each entry naming its resolver and guarded against going stale. A clip that is authored but deliberately never spoken (`pit-readback/windshield-off` — the readback never speaks the negative, since open-wheel cars have no windshield and telemetry cannot tell) is listed in `DELIBERATELY_UNSPOKEN` with its reason, and the test insists each entry is still authored and still unreferenced.

**Where the rules live since #1066.** The three rules — `coverageOf`, `orphansOf`, `danglingOf`, and `checkCoverage` over them — are `@iracedeck/callout-script`'s `coverage.ts`, pure over `{ script, authored, sharedClips }` where `authored` is a plain `<group>/<name>` list and `sharedClips` the plugin's built-ins (the runtime manifest's clips outside `voice/`, which a frame's `sfx/…` literals are checked against — the test reads them off the committed `manifest.json`), so that `pnpm lint:pack` (`packages/audio-scenarios/src/reference/lint-pack.ts`, run over the clip FILES a pack ships) is held to the very same rules as the bundled voice and cannot become a second source of truth. They could live in neither consumer: `@iracedeck/audio-scenarios` depends on this package, and this package's `src/generate/` is run by `tsx` and never built, so the grammar package both already hold is the one place both reach. `src/generate/script-coverage.ts` is the glue from a config to those rules: `authoredNamesOf(config)` (the `groups` as that list), `coverageOfConfig(config, sharedClips)` (through `buildCalloutScript`), and `unscriptedGroupsAreVarDriven(coverage)` — the var-driven reading a caller WITHOUT the vocabulary has, handed to `orphansOf` as its predicate. The test keeps `VAR_DRIVEN_BASES` and `DELIBERATELY_UNSPOKEN` because they are facts about the bundled voice, not rules, and holds every voice to `unusedAliases` being empty — a `pools` alias no live step names is a name that stopped carrying a decision; the linter reads the var-driven groups off the vocabulary's descriptions instead (`descriptionNamesGroup`) and carries no allowlist, so a lint of the bundled tree is not expected to be clean.
...
`manifest.json` is the **runtime manifest**, and since #1034 stage 3 it has a sibling: `generate:manifest` writes both, from one generator. `manifest.json` names every **authored** voice — every `voice/<id>/` directory — and is what this package's own generators and tests read as "the authored voice". `manifest.bundled.json` names only the **bundled** slice (`BUNDLED_VOICE_IDS`): the manifest a plugin compiles in, describing what the distributable itself provides — which is also where the reserved voice ids come from, the ones that stop a downloaded pack claiming a voice the plugin already has. They were identical until stage 3; since the flip the bundled slice is the `sfx/` clips and nothing else.

**The wiring since #1034 stage 3, task 2:** all three plugins (`iracing-plugin-stream-deck`, `iracing-plugin-mirabox`, `iracing-plugin-ulanzi`) import `@iracedeck/audio-assets/manifest.bundled.json` in `plugin.ts` — never `manifest.json` — so a plugin's compiled-in manifest can never outgrow what its own build actually copies. The scenario harness, this package's own generators/tests, and any other consumer that needs every authored voice import `manifest.json`.

The bundled manifest a plugin carries is only the **built-in half** of what that plugin plays: installed voice packs under `%LOCALAPPDATA%\iRaceDeck\Race Engineer\Voices` contribute their own clips at runtime, each pack acting as its own audio root, and the scenario engine consumes the union (`mergeManifests` in `@iracedeck/audio-scenarios`). So a voice missing from these files is not necessarily missing at runtime. They list clips only — never a voice's `callouts.json`, which ships beside the clips but is read by the voice-pack service, not resolved as a clip. Rebuild after adding/removing clips with:
...
5. Commit the new `voice/<voice>/<group>/<name>.mp3` clip(s) together with the regenerated manifests (`generate.manifest.json`, `manifest.json`, `manifest.bundled.json`) — clips are tracked in git.
...
5. Point the callout at its clips: for a scripted family the `sequence` step in the voice's `configs/<voice-id>.voice.json` addresses them as `pool:<group>/<base>` (the post-edit hook re-extracts the script in a Claude session; `pnpm generate:callout-scripts` by hand otherwise) — a `pools` entry only where a name carries a decision, an alias or a separate no-repeat tracker, with the reason in its `comment`. Every family is scripted since #1065, so there is no code-side pool to declare; the family's TypeScript file lists the `(group, base)` in its literal `<FAMILY>_CLIP_SOURCES`, and the coverage check above fails on a clip the script never references.
