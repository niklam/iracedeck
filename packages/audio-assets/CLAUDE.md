...
4. Author the voice's callout script — `scenarios`, `frames`, `pools`, `fragments` in the same config — and run `pnpm generate:callout-scripts` to extract `voice/<voice-id>/callouts.json`. Without one the voice is clips-only: it loads, appears in the dropdown, and says nothing, because absent means skipped. Commit the artifact with the config; the freshness test insists.
