...
**Where the rules live since #1066.** The three rules — `coverageOf`, `orphansOf`, `danglingOf`, and `checkCoverage` over them — are `@iracedeck/callout-script`'s `coverage.ts`, pure over `{ script, authored }` where `authored` is a plain `<group>/<name>` list, so that `pnpm lint:pack` (`packages/audio-scenarios/src/reference/lint-pack.ts`, run over the clip FILES a pack ships) is held to the very same rules as the bundled voice and cannot become a second source of truth. They could live in neither consumer: `@iracedeck/audio-scenarios` depends on this package, and this package's `src/generate/` is run by `tsx` and never built, so the grammar package both already hold is the one place both reach. `src/generate/script-coverage.ts` is the glue from a config to those rules: `authoredNamesOf(config)` (the `groups` as that list), `coverageOfConfig(config)` (through `buildCalloutScript`), and `unscriptedGroupsAreVarDriven(coverage)` — the var-driven reading a caller WITHOUT the vocabulary has, handed to `orphansOf` as its predicate. The test keeps `VAR_DRIVEN_BASES` and `DELIBERATELY_UNSPOKEN` because they are facts about the bundled voice, not rules; the linter reads the var-driven groups off the vocabulary's descriptions instead (`descriptionNamesGroup`) and carries no allowlist, so a lint of the bundled tree is not expected to be clean.
