...
├── rolling-start.ts          # Family: rolling-start (issue #660) — pace car begins rolling the field into the formation lap
...
> **`start-light` family (issues #480 / #673).** `start-lights.ts` exports `START_LIGHT_ALERTS` (6 scenarios), `START_LIGHT_SCENARIO_IDS`, and `START_LIGHT_POOL_NAMES`. Two gantry scenarios fire off `startLight.start-ready.raised` / `startLight.start-go.raised` (`WEIGHT.CRITICAL` + `interrupt:true`). The procedure is Ready → Set → Go: #673 moved the heads-up line ("Lights. Get ready to go.") from the `StartSet` edge to `StartReady` — Set lights too late to be useful, so nothing is spoken there (#666 had dropped the original `start-ready` cue on the mistaken assumption Ready follows Set). The diff's Ready emission is standing-only (rolling starts hold `StartReady` through the formation, where the rolling-start family owns the lead-in). Four countdown scenarios all subscribe to the **single** `startLight.countdown.raised { seconds }` event and filter `where: e.data.seconds === N` for N ∈ {90,60,30,10} — the "many scenarios → one event, one opt-in" shape (mirrors pit-box's `pitBox.countdown { mark }`), all `WEIGHT.NORMAL` + `queueable:false` (lowered from `SAFETY` by #666 — pre-race, low-stakes); every mark speaks exactly "[N] seconds to race start." (#673). The family carries **two grouped opt-ins** (`StartLightCalloutId = "lights" | "countdown"`, `START_LIGHT_CALLOUT_SETTING_KEYS = { lights: "calloutEnabledStartLights", countdown: "calloutEnabledStartCountdown" }`): the 2 gantry scenarios map to `lights`, the 4 countdown scenarios to `countdown` via `SCENARIO_ID_TO_START_LIGHT_ID`. `registerPitCrew` takes a `getStartLightCalloutEnabled` closure (default `() => true`) placed before the master-gate param, and wraps the scenarios with `wrapWithMaster(wrapCalloutScenario(...))`.

> **`rolling-start` family (issue #660).** `rolling-start.ts` exports `ROLLING_START_ALERTS` (1 scenario), `ROLLING_START_SCENARIO_IDS`, and `ROLLING_START_POOL_NAMES`. The single subject `pace-car` fires off `rollingStart.pace-car-moving.raised` (emitted by `sim-events-iracing`'s `diff/rolling-start.ts` on entry into the `ParadeLaps` SessionState, rolling-start only), at `WEIGHT.SAFETY`. It fills the start-of-formation gap #480 left — a standing start gets the gantry + countdown, but a rolling start had nothing until "one pace lap to go" near the END of formation (#657). It carries **one opt-in** (`RollingStartCalloutId = "pace-car"`, `ROLLING_START_CALLOUT_SETTING_KEYS = { "pace-car": "calloutEnabledRollingStartPaceCar" }`); `registerPitCrew` takes a `getRollingStartCalloutEnabled` closure (default `() => true`) placed before the start-light / master-gate params, and wraps the scenario with `wrapWithMaster(wrapCalloutScenario(...))`. Race-session + in-car gating lives on the scenario `where:` (`liveRaceCar`), not the diff, so the event stays harness-firable.

> **Expanded `flag` family (issue #480).** The `flag` family (`flag-alerts.ts`) gained 10 scenarios on top of the #467 colour flags: driver-black (`disqualify`, split out of `black` with its own "Disqualified. Pull off." line; `furled`; `dq-scoring-invalid`), race-progression (`crossed`, `one-pace-lap-to-go`, `green-held`, `ten-to-go`, `five-to-go`), and caution-waving (`yellow-waving`, `caution-waving` — separate, more-urgent lines from the static yellow scenarios, which are unchanged). All ride the existing `flag.*.raised` events with one new per-subject `calloutEnabledFlag*` opt-in each, so they slot into the existing flag closure with no new `registerPitCrew` param. (`one-pace-lap-to-go` was renamed from `one-lap-to-green` and re-triggered off a start/finish-crossing heuristic in `sim-events-iracing`'s `diff/pace-laps.ts` rather than the raw `OneLapToGreen` bit — issue #657 — but its audio wiring is unchanged.)

> **Yellow-cleared delivery + waving debounce (issue #671).** `flag.yellow.cleared` is a one-shot event, so `YELLOW_CLEARED` is `queueable: true` (like `FURLED`): a fire deferred behind an equal-weight non-flag line (spotter call, pit chatter) replays when the Voice bus idles instead of being silently dropped — the "no Yellow cleared heard" bug. The waving scenarios (`yellow-waving`, `caution-waving`) carry `cooldown: WAVING_FLAG_COOLDOWN_MS` (30 s, exported from `flag-alerts.ts`) because iRacing re-raises the waving bits on every re-approach of a persistent incident zone — the cooldown collapses the repeats to one callout per 30 s window, mirroring CrewChief's `timeBetweenYellowFlagMessages`.
...
- `resumable?:` (default `false`, requires `queueable: true` — validated at load time) — when an `interrupt` cuts this fire mid-playback, stash how far it got and CONTINUE from the interrupted clip at idle-replay instead of re-firing from the top (issue #758). The replay re-expands the sequence first: an unchanged expansion resumes at the interrupted clip (re-keying the radio frame with the open tick when one was already opened before the cut); a changed expansion — the snapshot moved on while stashed — falls back to a full fresh replay, preserving the #481 freshness guarantee. Because the resume decision re-expands, a resumable scenario's `if:` predicates must be side-effect-free, and pool draws may differ between expansions (forcing the full-replay path) — best suited to deterministic sequences like the pit-service readback.
- `pendingHoldMs?:` — after this fire finishes, hold the bus's pending replay for N ms instead of draining it immediately (issue #758). Declared by scenarios that arrive in a train of related fires (the pit-box count-in marks, ~1 s apart) so the line they displaced doesn't stutter back into the gaps between family-mates; the hold re-arms after each member and the displaced fire plays once the train has been quiet for the window. A new fire taking the bus cancels the hold.
