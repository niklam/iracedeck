...
- `script-compiler.ts` — `compileVoiceScript(script, deps)` (issue #1064): pure, never throws; turns one voice's `callouts.json` into `ResolvedStep` trees checked against the engine's contracts, vars, conditions, cases, pools and legacy fragments, and returns `skipped` diagnostics instead of raising them (`deliberate: true` for `skip: true` and absent entries; a reason naming the reference otherwise — `unknown pool "x"`, `case "x": unknown key "k"`, `unknown frame "x"`, `no contract`). String steps go through the DSL's own `parseStepShorthand`, so a script and a closure sequence that spell the same thing resolve identically (`dsl.test.ts` pins the DSL's parser against the grammar's `parseStringStep`). A `!cond` compiles to a negating predicate that is deliberately NOT try/caught here — the interpreter's `if` arm logs a throw and reads it as false. Exported from the package index for #1066's `lint:pack`.
- `interpreter.ts` — the scenario engine: subscribes scenarios AND contracts to the bus (`defineScenario` / `defineContract` share one registration path), expands sequences at fire time (`pickFromPool` with per-pool no-immediate-repeat, `{voice}` substitution, include/var/conditional/`case` resolution), and runs the weight/interrupt/queueable scheduler with per-bus focus floors and the single pending slot. Pools register either as manifest-derived (`definePoolFromManifest(name, group, base)` — members are every `voice/<voice>/<group>/<base>-NN.mp3` in the manifest, resolved per active voice at fire time; issue #664) or as explicit clip lists (`definePool`); since #1064 a pool the **active voice's script** defines shadows a code pool of the same name for that voice only (state cached per `(voice, name)`, cleared by `setManifest` and every compile), and a slashed pool step (`group/base`) routes to `pickFromPoolRef`. Expansion runs **before** bus scheduling (`prepareOps`), and a required step resolving to nothing aborts the whole callout (issue #835 — see "Required-step abort" below). The script seam (#1064): `defineVar(name, resolver, description?)` / `defineCond(name, predicate, description)` / `defineCase(name, resolver, keys, description)` are the vocabulary registries, `vocabulary()` reports them, `setScripts(voiceId → CalloutScript)` compiles every voice eagerly (one parameter-free `Voice scripts loaded` at info; per voice at debug `Voice "<id>": <n> of <m> callouts scripted` plus the deliberate skips; ONE warn per (voice, scenario) for a skip the pack did not mean, deduped across recompiles); any later registration marks the compiled map dirty and the next `prepareOps` recompiles first. At fire time a contract looks up `compiled.get(activeVoice)?.scenarios.get(id)` — missing means silent: no ops, no cooldown, no bus take. **Frames**: a body that expanded to at least one `play` op is wrapped in the frame the script entry names (else the contract's `frame`, else `DEFAULT_FRAME`), expanded from the active voice's compiled `frames` with `base: undefined` and the scenario's channel; `getFrameOptions()` (the sixth `initializeAudioScenarios` argument, default both true) is read at every frame expansion and drops the frame's non-`ambient` ops when `beeps` is off and its `ambient` ops when `ambience` is off. A legacy `Scenario` is framed the same way when its voice has a compiled script, and plays unframed (debug) when it has none; a voice whose script lacks the frame a legacy scenario names warns once per (voice, frame) and plays it unframed. A frame step that resolves to nothing aborts the callout like any required step — and since a broken frame silences EVERY callout it wraps, that abort also warns once per (voice, frame), naming the reason (cleared by `setScripts`); the per-fire detail stays at debug. `applyFrame` is the method; `prepareOps` calls it right after the body expands.
