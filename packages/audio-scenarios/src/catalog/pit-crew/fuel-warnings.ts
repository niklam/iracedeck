 ...
 *
 * Weight mapping (preserved from the legacy pit-engineer):
 *   - 5 laps: default weight (`WEIGHT.NORMAL`) — advisory, pit-lane messages
 *     still take precedence.
 *   - 3 / 1 / 0 laps: `weight: WEIGHT.CRITICAL` + `interrupt: true` — these
 *     callouts cut lesser in-flight scenarios because the driver needs to
...
function fuelScenario(id: string, threshold: number, pool: string, options: { urgent: boolean }): Scenario {
  return {
    ...
    bus: AudioBus.Voice,
    base: "pit-crew",
    weight: options.urgent ? WEIGHT.CRITICAL : WEIGHT.NORMAL,
    interrupt: options.urgent ? true : undefined,
