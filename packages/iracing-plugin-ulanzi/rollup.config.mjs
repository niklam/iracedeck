...
/**
 * Rollup plugin to copy static assets for the Ulanzi plugin.
 ...
 * - PI browser assets (`sdpi-components.js`, `pi-components.js`, `ulanzi-pi-bridge.js`) come from `@iracedeck/pi-components`.
 */
function copyAssetsPlugin(sdPlugin) {
  return {
    ...
    generateBundle() {
      ...
      for (const jsFile of ["sdpi-components.js", "pi-components.js", "ulanzi-pi-bridge.js", SETTINGS_WINDOW_BRIDGE]) {
        const src = path.join(browserDir, jsFile);
        if (!existsSync(src)) {
          this.error(
            `Missing ${jsFile} in @iracedeck/pi-components. Build it first: pnpm --filter @iracedeck/pi-components build`,
          );
        }
...
const sdPlugin = "com.ulanzi.iracedeck.ulanziPlugin";
...
const config = {
  ...
  external: ["@iracedeck/audio-native", "@iracedeck/iracing-native", "@resvg/resvg-js", "yaml", "keysender", "ws"],
  plugins: [
