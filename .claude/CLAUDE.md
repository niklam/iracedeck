...
- `@iracedeck/sim-events-iracing` — the iRacing translator: diffs `sdkController` ticks and publishes semantic events, one diff module per family under `src/diff/`. The ONLY package that consumes `@iracedeck/iracing-sdk` telemetry; a future sim adapter is a parallel sibling emitting the same catalog. Owns the canonical live order (`getLiveRacePositions`/`getLivePosition`, `race-order.ts`) — see `race-positions.md`. Also records every car's lap crossings for the replay store (`replay-laps`, `replay.lapStarted` / `replay.lapTimed`, #1203).
