...
- `@iracedeck/deck-core` — platform-agnostic base classes, types and services. Has its own `CLAUDE.md`; the settings store and window are in `global-settings.md` / `settings-window.md`, window focus in `keyboard-shortcuts.md`, the development voice root in `platform-feature-flags.md`. It also hosts the voice-pack stack (`voice-pack-{scanner,service,installer,launch}.ts`): packs under `%LOCALAPPDATA%\iRaceDeck\Race Engineer\Voices`, each voice's `callouts.json` read beside its clips and handed to the engine, voices addressed as `<pack id>::<voice id>` (#1144; ours is `default::default`) so two packs may ship the same bare voice id and the scanner refuses only a voice id declared twice within one pack. No plugin bundles a voice since #1034 stage 3: the launch step keeps the managed `default` pack at the catalog's digest (and other catalog packs up to date) on every start, silently, retrying on a schedule that tightens while the Race Engineer is on. The `voice-script-missing` banner comes from `voice-script-warning{,-reporter}.ts`.
