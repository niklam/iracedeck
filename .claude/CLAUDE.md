...
- `@iracedeck/iracing-plugin-ulanzi` — Registers actions from `@iracedeck/iracing-actions` via `UlanziPlatformAdapter` for Ulanzi Deck devices (D200 / D200H / Dial / D200X). Wraps action UUIDs into the required `com.ulanzi.ulanzistudio.*` namespace and injects a PI WebSocket bridge (`ulanzi-pi-bridge.js`, from `@iracedeck/pi-components`) so the shared sdpi-components PI works over UlanziStudio's protocol. Has its own `CLAUDE.md`. Not yet hardware-validated.
- `@iracedeck/website`
