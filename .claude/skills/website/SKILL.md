...
| `packages/website/src/pages/404.astro` | The site's own 404 page, rendered with `StarlightPage` because `disable404Route: true` (Starlight 0.42's injected route warns about a missing `docs/404` entry, and a `404.md` entry collides with the `[...slug]` route; see the file's header comment) |
