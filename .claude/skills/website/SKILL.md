...
| `packages/website/src/pack-reference-types.ts` | The voice-pack reference artifact's shape (`src/data/pack-reference.json`, #1066), re-declared by hand with a path-naming runtime parser — the website must not depend on `@iracedeck/audio-scenarios`, whose `src/reference/pack-reference.ts` is the source it mirrors. `pack-reference-view.ts` holds the pure helpers the three `PackReference*.astro` components and `routeData.ts` (injected "On this page" entries) share. |
