...
| Fuel Dial | 1 | single action, no sub-action dropdown (`encoder: true`, telemetry-aware) — Stream Deck+ dial that is **modal** (manual vs autofuel, derived live from `dpFuelAutoFillActive`, not stored). A bare turn in **manual** mode sets fuel via a Mode setting: Add Amount (amount to add) or Target Amount (desired total after the stop, the needed add (target − current) rounded up so you never finish under target, recomputed continuously as fuel burns and the request updated whenever the whole-unit amount changes **while fuel-fill is on** — while fuel-fill is off the fuel amount is not updated, though rotating still plans the target and turning fueling on/pressing sends it; marked by a red target line on the bar). A bare turn in **autofuel** mode adjusts the autofuel lap margin via the `fuelServiceLapMarginIncrease`/`Decrease` key bindings (coalesced). Manual fuel sends iRacing API `pit.fuel` (the rounded-up amount needed to reach the target; may exceed the current remaining tank space since more fuel burns before the stop and iRacing fills only up to capacity) with a live touch-strip readout and a two-segment fuel bar (current + add over capacity; green add when fueling is on, gray otherwise); **encoder-only** — no keypad placement (`Controllers: ["Encoder"]` / `["Knob"]`; keypad fuel control is Fuel Service's job) — and also works on a Mirabox knob. The touch-strip slot is fully self-drawn with a full-width tri-state status band across the TOP (#728) — green "REFUEL: ON" / red "REFUEL: OFF" in manual mode, "AUTOFUEL: ON"/"AUTOFUEL: OFF"/gray "AUTOFUEL: N/A" in autofuel mode — the toggle buttons' status-bar language moved to the top. The strip uses a single full-canvas pixmap encoder layout (`layouts/fuel-dial.json`, `box` key, like Setup Brakes Dial) because built-in layout text items can't have a colored background; texts are positioned by explicit baselines (the deck app's QT renderer ignores `dominant-baseline`). Five configurable gesture slots — Push (default Toggle Fueling), Long Press (default Toggle Autofuel, all platforms), Push + Turn (Full / No Fuel — CW fills, CCW empties — or None), Tap Display and Long Touch (default None, Elgato-only touch strip) — each over {Toggle Fueling, Toggle Full / No Fuel, Toggle Autofuel, Switch Mode, None}. Toggle Autofuel taps `fuelServiceToggleAutofuel`; Toggle Fueling and Toggle Full / No Fuel use the iRacing API; Switch Mode talks to nothing (it flips the Mode setting between Add Amount and Target Amount). Mode, Step Size, the five gesture slots, and Units are settings, not sub-actions. Press/long-press/push+turn are classified at `dialUp` (no timer). Toggle Full / No Fuel is a toggle (full ↔ no fuel; the no-fuel side resets the requested amount to nothing). The readout refreshes every 5 s and immediately on a displayed-state change. Toggle reads the live `PitSvFlags`/`FuelFill` checkbox. First dial-capable action of the #681 rebuild |
