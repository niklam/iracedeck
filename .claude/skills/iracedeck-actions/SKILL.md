...
Dial/encoder support was de-claimed in issue #640 pending a rebuild; that rebuild has begun in issue #681. **Fuel Service** (`encoder: true`) carries the first and reference dial surface (the former standalone Fuel Dial action, merged into Fuel Service in #759) — on a Stream Deck+ dial + touchscreen (and a Mirabox knob) it is the fuel dial, on a keypad button it is the modal fuel action. **Setup Brakes Dial** (`encoder: true`, #730) is a Stream Deck+ dial (Elgato only) that adjusts one brake setup value by turning. **Audio Controls** (`encoder: true`, #782) carries the third dial surface — on a dial/knob, rotation adjusts the selected audio's volume and the press is configurable as push-to-talk or mute/unmute. Every other action remains keypad-only (`encoder: false`) until it is individually rebuilt. See `.claude/rules/encoders-and-touchscreen.md` for the per-action dial rules.
