...
| Fuel Dial | 1 | single action, no sub-action dropdown (`encoder: true`, telemetry-aware) — Stream Deck+ dial sets fuel via a Mode setting: Add Amount (amount to add) or Fill To (desired total after the stop, rounded up with a fixed +1 L safety buffer so you finish at least the target, and kept topped up every 30 s **while fuel-fill is on** — while fuel-fill is off the fuel amount is not updated, though rotating still plans the target and turning fueling on/pressing sends it; marked by a red target line on the bar). Sends iRacing API `pit.fuel` (clamped to remaining tank space) with a live touch-strip readout and a two-segment fuel bar (current + add over capacity); also works as a plain keypad button and a Mirabox knob. The mode-aware title reads "Add Fuel" / "Fuel Target". Mode, Step Size, Press Action / Long-Press Action / Touch Screen action (Toggle Fueling / Clear Fueling / Fill To Max / None) and Units are settings, not sub-actions. Fill To Max is a toggle (full ↔ ~empty 1 L). The readout refreshes every 5 s and immediately on a displayed-state change. Rotating always sets the amount and arms fueling via the API. Toggle reads the live `PitSvFlags`/`FuelFill` checkbox. First dial-capable action of the #681 rebuild |
