...
### Issue labels: kind, area, priority (2026-09-05)

Three more axes sit beside `bug` / `enhancement`, one value each, so the backlog can be sorted by what the work is, where it lands, and how much it matters. `bug` / `enhancement` keep saying "user-facing fix" / "user-facing feature"; the `kind:` axis names the work that is NOT user-facing.
...
| `p1` / `p2` / `p3` | `p1` next up: blocks a release or a user, or the maintainer's committed order; `p2` planned: this or next release; `p3` backlog: fine to sit | **Niklas only.** An agent proposes a priority in the issue text or in conversation and never applies the label |

Milestones keep saying *when*; priority says *how much*. `discord` (a mirrored Discord request), `device`, `breaking change` and `approved` are unchanged and orthogonal. The backfill over the open backlog was done on 2026-09-05; a new issue gets its `kind:` (when it has one) and `area:` at filing.

**The board.** The *iRaceDeck Roadmap* project (`https://github.com/users/niklam/projects/1`, `gh project … 1 --owner niklam`) is a view over the same issues, never a second record: its `Status` lanes are `Backlog` (p3), `Planned` (p2), `Next` (p1, the agreed order top first), `In progress` (a worktree exists), `Testing` (waiting on the manual test or CodeRabbit), `Done` (merged) and `Dropped` (closed as not planned or duplicate). Every closed issue is on it too, with `area:` and `kind:` but no priority, so the board is the history as well as the plan. Priority labels and lanes say the same thing from two sides; when they disagree the label is the record and the lane is stale. An agent adds a new issue to the board at filing (`gh project item-add 1 --owner niklam --url <issue url>`, then `gh project item-edit` into the lane its priority implies), moves it to `In progress` when the worktree is created and to `Testing` when handing over for the manual test; a merged issue goes to `Done`, one closed without shipping to `Dropped`. Lanes need the `project` token scope (`gh auth refresh -s project,read:project`). Never rewrite the `Status` field's option list through the API: `updateProjectV2Field` replaces the options and regenerates their ids, which clears every card's lane — add a lane in the UI instead.
