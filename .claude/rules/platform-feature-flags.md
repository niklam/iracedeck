...
pnpm build                    # stages every pack into this worktree's default root, then builds the plugins
# restart the plugin, or press "Rescan voices" in iRaceDeck Settings, then drive
```

There is no watcher on the root and no automatic rescan (#1214 §4): the plugin scans at every start, and **Rescan voices** covers the case where you do not want to restart.

`pnpm dev:voices` sets the per-worktree override, and prints what the build will then decide:

- `on` writes the default root. A marker already holding a hand-picked path is kept and reported.
