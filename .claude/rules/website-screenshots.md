...
- **Deterministic.** The capture serves a fixed settings fixture (`scripts/lib/settings-window-capture/seed.mjs`), so reruns differ only where the UI actually changed. Hand-captured shots differ on every unrelated detail, which makes review impossible.
