...
- **Where users flip it:** the "Enable debug logging" checkbox on the settings window's Diagnostics tab (shared partial `global-common-diagnostics.ejs`). Since #1003 it is not in any action Property Inspector — when a support thread says "turn on debug logging", point the user at the window's **Open iRaceDeck Settings** button.
