...
There is deliberately no flake caveat here. Measured on 2026-09-01, the last 60 `ci-test` runs were 58 successes, one `action_required`, and one genuine test failure on a development branch — first attempt, fixed by later commits rather than by a re-run. That tally cannot see a flake, though: `gh run list` reports a run at its latest attempt, so a failure that passed on a re-run counts as a success. Two runs inside that very window did exactly that, and both were the same flake — `settings-store.test.ts` (#1088), on 2026-08-28 and again on 2026-09-01. A green history is therefore not evidence that reds are usually spurious; it is evidence that the instrument cannot tell. Treat a red as real until shown otherwise, and re-run to diagnose rather than to clear.

Then remove the worktree — unless a deck host is linked to it. **Never relink a host on your own, not even back to `master`** (Niklas, 2026-09-25): the link is his to point, and a relink swaps the plugin out from under whatever he or another session is testing. Leave the link and the worktree as they are and tell him; removing the tree under a live link would silently break his installed plugin, which is why the hook refuses it.
