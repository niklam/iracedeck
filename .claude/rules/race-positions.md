...
- **Physical track order is a different concept, with its own single helper.** "The car physically ahead / behind on the road" (`CarIdxLapDistPct` circular distance, regardless of lap count or standings) is NOT a race position, so the canonical-order rule does not apply to it — but the same don't-recompute discipline does: `findNearestCarOnTrack` (`@iracedeck/iracing-sdk` `track-utils.ts`) is the only track-order primitive. Replay Control's dial and the Camera Controls dial's track-order mode (via `computeTrackOrderTarget` in `iracing-actions/src/shared/car-cycling.ts`, #886) both consume it; never sort cars by lap distance yourself.
