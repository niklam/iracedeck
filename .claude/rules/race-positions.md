...
- **Official counters are fallback only.** `CarIdxPosition` / `CarIdxClassPosition` are used per-car **only** when there is no live order — non-race sessions (where lap-progress order isn't the standings) or before the order is available. A non-positive (`0` = not classified) official value renders blank, not `"0"`.
