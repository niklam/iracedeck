...
**`ird-voice-select` / `ird-name-select`** - The Race Engineer voice and driver-name dropdowns, populated from the `_raceEngineerVoices` / `_driverNames` runtime-pushed globals. **They select a fallback when the saved value is absent from the list, but persist it only when there was none to begin with** (#1034) — unlike `ird-audio-device-select` above, and deliberately so: since voice packs, those lists are a function of what is on disk at scan time, so a saved voice can leave the list because a pack folder was briefly unreadable. Writing the fallback then would destroy the user's choice on one press of Rescan. The plugin resolves the same situation read-only (`resolveActiveRaceEngineerVoice` never writes), so leaving the setting alone keeps the two in agreement and restores the choice when the pack returns. The one write the plugin does make is `migrateRaceEngineerVoiceId` (#1144), which only rewrites a bare stored id into the composite it meant and never replaces a value with a fallback (`@.claude/rules/global-settings.md`). `_raceEngineerVoices` lists composite ids (`<pack id>::<voice id>`) and the partial's `default` is `default::default`; an entry `_voiceLabels` does not name falls back to the title-cased voice half of its id (`default::default` → `Default`), never to the raw composite. `ird-voice-select` keeps a copy of the `::` separator for the reason `ird-name-select` keeps one of `TAKE_SUFFIX` below, and `voice-select.test.ts` pins it to `@iracedeck/callout-script`'s `VOICE_ID_SEPARATOR`. `_driverNames` lists name BASES — `scanDriverNames` folds a pack's name takes (`niklas-01`) onto their base with `@iracedeck/callout-script`'s `stripTakeSuffix`, the engine's own take rule (#1173) — and a stored take shows in `ird-name-select` as its listed base without being persisted, the same read `resolveActiveDriverName` makes at playback. The component keeps a copy of `TAKE_SUFFIX`, because the PI bundle cannot import the package; `name-select.test.ts` pins the copy to the shared rule.
