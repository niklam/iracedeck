...
**`ird-voice-select` / `ird-name-select`** - The Race Engineer voice and driver-name dropdowns, populated from the `_raceEngineerVoices` / `_driverNames` runtime-pushed globals. **They select a fallback when the saved value is absent from the list, but persist it only when there was none to begin with** (#1034) — unlike `ird-audio-device-select` above, and deliberately so: since voice packs, those lists are a function of what is on disk at scan time, so a saved voice can leave the list because a pack folder was briefly unreadable. Writing the fallback then would destroy the user's choice on one press of Rescan. The plugin resolves the same situation read-only (`resolveActiveRaceEngineerVoice` never writes), so leaving the setting alone keeps the two in agreement and restores the choice when the pack returns. `_driverNames` lists name BASES — `scanDriverNames` folds a pack's name takes (`niklas-01`) onto their base with `@iracedeck/callout-script`'s `stripTakeSuffix`, the engine's own take rule (#1173) — and a stored take shows in `ird-name-select` as its listed base without being persisted, the same read `resolveActiveDriverName` makes at playback. The component keeps a copy of `TAKE_SUFFIX`, because the PI bundle cannot import the package; `name-select.test.ts` pins the copy to the shared rule.
