...
**`ird-voice-select` / `ird-name-select`** - The Race Engineer voice and driver-name dropdowns, populated from the `_raceEngineerVoices` / `_driverNames` runtime-pushed globals. **They select a fallback when the saved value is absent from the list, but persist it only when there was none to begin with** (#1034) — unlike `ird-audio-device-select` above, and deliberately so: since voice packs, those lists are a function of what is on disk at scan time, so a saved voice can leave the list because a pack folder was briefly unreadable. Writing the fallback then would destroy the user's choice on one press of Rescan. The plugin resolves the same situation read-only (`resolveActiveRaceEngineerVoice` never writes), so leaving the setting alone keeps the two in agreement and restores the choice when the pack returns. The one write the plugin does make is `migrateRaceEngineerVoiceId` (#1144), which only rewrites a bare stored id into the composite it meant and never replaces a value with a fallback (`@.claude/rules/global-settings.md`). `_raceEngineerVoices` lists composite ids (`<pack id>::<voice id>`) and the partial's `default` is `default::default`; an entry `_voiceLabels` does not name falls back to the title-cased voice half of its id (`default::default` → `Default`), never to the raw composite. A stored bare id that is not itself listed shows in `ird-voice-select` as the composite `qualifyVoiceId` reads it as — the managed pack being the one its `default` attribute names — without being persisted, the same read `resolveActiveRaceEngineerVoice` makes before it falls back, so the dropdown never shows a different voice from the one that plays while the migration waits for the managed pack. `ird-voice-select` keeps copies of the `::` separator and of `qualifyVoiceId` for the reason `ird-name-select` keeps one of `TAKE_SUFFIX` below, and `voice-select.test.ts` pins them to `@iracedeck/callout-script`'s `VOICE_ID_SEPARATOR` and `qualifyVoiceId`. `_driverNames` lists name BASES — `scanDriverNames` folds a pack's name takes (`niklas-01`) onto their base with `@iracedeck/callout-script`'s `stripTakeSuffix`, the engine's own take rule (#1173) — and a stored take shows in `ird-name-select` as its listed base without being persisted, the same read `resolveActiveDriverName` makes at playback. The component keeps a copy of `TAKE_SUFFIX`, because the PI bundle cannot import the package; `name-select.test.ts` pins the copy to the shared rule.

**`ird-voice-pack-list` / `ird-voice-pack-refresh`** - The Installed Voices list and its Rescan button (#1034), settings-window only. The list renders the run-scoped `_voicePacks` global — `{ packs, problems }`, the whole result of the last scan, so a pack that is present but ignored shows the reason instead of merely being absent. Every cell is `textContent`: on the sideload path a pack's folder name, label and manifest are a file a third party wrote. The button sends `sendToPlugin {event:"voicePackRefresh"}`, handled by the settings-window command handler — which is why both are gated on `locals.settingsWindow` (a PI's `sendToPlugin` goes to its own action instead).
...
**`ird-enable-feature`** - One-press opt-in on the Getting Started page (#1061), `feature="race-engineer" | "changelog-updates" | "focus-iracing-window"`. **State-driven, never fire-and-forget:** it reads the setting it would change and renders either the offer or the current truth, so pressing twice is not a case that exists and somebody who enabled the feature elsewhere is told so. It writes nothing itself — the press sends `sendToPlugin { event: "enableFeature", feature }` and the plugin decides, which is what lets the Race Engineer opt-in write its gate and its startup policy together. It reads settings exactly twice — once on connect, and again on every `didReceiveGlobalSettings` push — and **must never re-read in response to a DOM event.** A document-scoped `change` listener calling `getGlobalSettings()` shipped briefly and broke every control on the settings window: in sdpi a read is not addressed to the requester, so the reply replaces the shared settings snapshot and every `global` control re-renders from it, while writes are debounced 250 ms — the read lands inside the window before the write is even sent. `enable-feature.test.ts` pins the absence by counting reads. The accepted cost is that the fake host does not echo a write back to the socket that sent it, so a sibling control on another tab flipping the same key leaves this one showing a stale offer until the next push; pressing it then is idempotent. Settings window only.
