...
- **Materials**: primary "metal" artwork = per-file `mtl` gradient whose stops are `{{graphic1Color}}` at opacity 1 → 0.55 (recolors with user overrides while keeping the shaded look); the eye sclera / chat bubble use the same technique via `scl2` (1 → 0.76). Fixed literal gradients for semantics: amber `ambG`, red `redG`, green `plusG`, minus `minusG`, lens glass `lns`, dark display `dgl`, tire `tireG`. Accent = `{{graphic2Color}}` (default `#4fc3f7` cyan; chat defaults `#2563ab`) marking the adjusted dimension — list `graphic2Color` in the action's PI `color-overrides` slots when a set uses it. Glow = duplicated element under a `feGaussianBlur stdDeviation="3"` filter copy.
...
- **Direction in the artwork, not chips**: when the drawing can show the direction it must, and the ± pair then carries **no** chip — pitch arrows, FOV cone width, horizon line height, driver-height/UI-size before→after, look-direction iris, FFB force (arc weight + Nm weight size), LFE/bass/haptic volume (wave count: louder three waves, quieter the two nearest the source, all waves solid `{{graphic2Color}}`), intensity (sine amplitude). Direction-aware ± pairs share one viewBox frame (union of both variants' bounds) with each variant horizontally centered, so both keys render at identical scale (the emitter's `pair:` key). Abstract ± modes with no natural directional drawing (camera-editor latitude/longitude/altitude/mic-gain etc., cockpit dash pages) keep the bottom-right rounded-square chip (26×26, rx 8, green `+` / red `−`, ringed with `{{backgroundColor}}`); the flat car sets use bare oversized `{{graphic1Color}}` +/− marks instead (stroke 6, no chip).
...
- **Flat car style**: car-bearing icons (Setup Aero, Setup Chassis, reload-car-textures) draw the shared flat top-view race car (the Tire Service car body verbatim) as `{{graphic1Color}}` on the background, one `{{graphic2Color}}` accent on the adjusted part, and the bare +/− marks above.
