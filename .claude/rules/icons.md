...
- **± marks**: no chips anywhere. When the drawing itself shows the direction (pitch arrow, FOV cone, wave count, before→after), the pair carries no mark at all. Otherwise the mark is a bare `{{graphic1Color}}` `+`/`−` (stroke 6, 24-unit arms) placed to the right of the artwork and **vertically centered on it**. Dash pages use `>` (next, right side) / `<` (previous, LEFT side of the panel) chevrons instead. When the graphic draws the ±, the title's second line must be a word (INCREASE/DECREASE), never a `+`/`−` sign.
