...
- **settings-window-getting-started.ejs** - The Getting Started page (#1061), rendered from the generated `data/getting-started.json`. Settings window only. Maps each `<!-- ird:action <id> -->` marker in the source page to its control and **throws at build time** on an id it does not know; the Profiles jump is gated on `locals.platform?.features?.profiles`. Regenerate with `pnpm generate:getting-started-data` after editing the source page — a freshness test enforces it.
