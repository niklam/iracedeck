...
A direct send focuses iRacing exactly like any other keystroke: since #977 the four press-emitting methods above call `focusIRacingBeforeInput()` themselves (`releaseKeyCombination` does not). So a caller must never add its own `focusIRacingBeforeInput()` — the service owns it, and a second call only asks the native focuser again for a window it has just focused.
