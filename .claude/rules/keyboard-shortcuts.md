...
`focusIRacingWindow` is a three-value **mode** since #977 — `always` (default), `required`, `never` — read through `getGlobalSettings().focusIRacingWindow` (the schema folds the pre-#977 boolean in: `true` → `always`, `false` → `never`). Two call sites serve it, both in `window-focus-service.ts`. `focusIRacingIfEnabled()` is the adapter-level site, registered as a listener on the adapter's key/dial events in every `plugin.ts`, and runs under `always` only. `focusIRacingBeforeInput()` is the keystroke site and runs under `always` and `required`: the binding dispatcher calls it immediately before each keyboard send (`tapKeyboard`, the `hold` keyboard branch, `tapSequence` after every binding resolved — never before a SimHub role, which goes out over TCP), and `ChatCommand.sendMessage` calls it through the `beforeKeystrokes` hook `createSDK` accepts (`iracing-sdk` cannot import deck-core, so deck-core's `initializeSDK` injects it; `macro()` and the other chat broadcasts take no hook). Under `always` a keybind press therefore asks twice, the second returning `AlreadyFocused` — that second ask is what covers Stream Deck+ touch-strip gestures (#978) without an `onTouchTap` on `IDeckPlatformAdapter`. The comms catalog (#612) is deliberately not consulted: whatever reaches those two paths is a keystroke by definition. Since #930 the default is on, so the adapter site runs on essentially every press — hence two logging rules: a `WindowNotFound` result logs at `warn` only when `isIRacingActive()` says iRacing is running and at `debug` otherwise, and `FocusTimedOut` warns once per episode then drops to `debug` until a focus succeeds (its usual cause, an elevation mismatch, makes *every* press time out). Both sites gate on `isSettingsStoreReady()`, **not** `isGlobalSettingsInitialized()`: the latter flips true before the settings store has loaded, while the cache is still schema defaults — which say `always` — so it would override an explicit `never` during startup. Note what focusing does and doesn't fix: keystrokes (keybind and chat actions) go to the focused window, but pure SDK broadcasts use `SendNotifyMessage(HWND_BROADCAST, …)` and arrive regardless of focus — those fail only on an integrity-level mismatch, which is what the elevation probe above covers.
