...
That policy also owns the one non-atomic path (#962), and it chooses the path up front from the bindings rather than by reading `tapSequence`'s `false`. The prime matches the target's binding kind first: a keyboard target prefers a keyboard-bound prime (so both can batch), a SimHub target a SimHub-bound one (it is faster: a keyboard tap focuses iRacing and holds ~100 ms natively, a local SimHub start/stop takes milliseconds), and either falls back to the other kind. When the target and the prime are both keyboard-bound it calls `tapSequence`, and a `false` there stays a skip — a keyboard-only setup never falls back to separate taps, so #818's no-flash guarantee holds. When a SimHub role is involved it sends a serialized `tap(prime)` then `tap(target)`, pressing the target only if the prime went out, since a lone target press would toggle an already-shown box off. Before pressing anything it checks `isSimHubReachable()` and skips while SimHub is offline, so no press waits out an HTTP timeout and a keyboard prime never toggles a box the SimHub target cannot follow. Every `showBlackBox` call is queued behind the previous one, so two dispatches in flight never interleave their presses. The building blocks are deck-core's: `IBindingDispatcher.tap` / `tapBinding` return whether the press went out, and `isKeyboardBound` (`isBindingKeyboardBound` on actions) sits beside `isConfigured`. `tapSequence` itself still refuses, never degrades.
...
A direct send focuses iRacing exactly like any other keystroke: since #977 the four sends above go through `focusIRacingBeforeInput()` on their way out (`releaseKeyCombination` does not). So a caller must never add its own `focusIRacingBeforeInput()` — the service owns it, and a second call only asks the native focuser again for a window it has just focused.
...
3. Call `initWindowFocus()` to set up window focusing — passing the app monitor's `isIRacingActive` as its third argument (#1176, see below) — and `initMousePointer()` for mouse-pointer placement
...
// isIRacingActive picks warn vs debug for a missing window. Injected, never
// imported inside deck-core: that import closed a cycle through the SDK singleton (#1176).
...
`focusIRacingWindow` is a three-value **mode** since #977 — `always` (default), `required`, `never` — read through `getGlobalSettings().focusIRacingWindow` (the schema folds the pre-#977 boolean in: `true` → `always`, `false` → `never`). Two call sites serve it, both in `window-focus-service.ts`. `focusIRacingIfEnabled()` is the adapter-level site, registered as a listener on the adapter's key/dial events in every `plugin.ts`, and runs under `always` only. `focusIRacingBeforeInput()` is the keystroke site and runs under `always` and `required`. The **keyboard service** is what calls it, immediately before each native emit — in `sendKey`, and in the scan-code and keysender helpers that `sendKeyCombination`, `pressKeyCombination` and `sendKeySequence` route through. Sitting at the emit rather than at the method entry is what makes the skips honest: a `sendKeySequence` abandoned for a missing sequence sender or an unmapped key focuses nothing, and `releaseKeyCombination` never focuses at all — the helper both halves share asks only on the press, since the press it pairs with has already brought the window forward. That layer rather than the binding dispatcher is deliberate: a SimHub role never reaches the keyboard service (the dispatcher hands it to SimHub over TCP, so it can never focus), while the two actions that bypass the dispatcher and call `getKeyboard()` directly — Race Admin's Ctrl+V paste into the chat box and Car Control's Escape — are covered without knowing it. Every keystroke deck-core emits passes through that one layer; the dispatcher's keyboard branches did not. `ChatCommand.sendMessage` calls it through the `beforeKeystrokes` hook `createSDK` accepts (`iracing-sdk` cannot import deck-core, so deck-core's `initializeSDK` injects it; `macro()` and the other chat broadcasts take no hook), and the Chat action's **Open Chat** and **Reply** modes call it themselves before their broadcast — those open a prompt for the driver to type into, so the sim has to be in front even though the action sends no key of its own. Under `always` a keybind press therefore asks twice, the second returning `AlreadyFocused` — that second ask is what covers Stream Deck+ touch-strip gestures (#978) without an `onTouchTap` on `IDeckPlatformAdapter`. The comms catalog (#612) is deliberately not consulted: what the keyboard service emits and what the chat send types are keystrokes by definition, and the catalog would be a second source of truth for what the call site already knows. Since #930 the default is on, so the adapter site runs on essentially every press — hence two logging rules: a `WindowNotFound` result logs at `warn` only when `isIRacingActive()` says iRacing is running and at `debug` otherwise (the plugin hands that check to `initWindowFocus` rather than the service importing `app-monitor`, which imports the SDK singleton, which imports the service for the chat hook — the cycle #1176 broke; a check that throws reads as not running), and `FocusTimedOut` warns once per episode then drops to `debug` until a focus succeeds (its usual cause, an elevation mismatch, makes *every* press time out). A timed-out ask also costs the native focuser's full ~1 s wait, so after one both sites skip the native call for `FOCUS_TIMEOUT_COOLDOWN_MS` (2000 ms): without that cooldown the doubled ask under `always` would stall a press for two seconds rather than one, for as long as the mismatch lasts. Both sites gate on `isSettingsStoreReady()`, **not** `isGlobalSettingsInitialized()`: the latter flips true before the settings store has loaded, while the cache is still schema defaults — which say `always` — so it would override an explicit `never` during startup. Note what focusing does and doesn't fix: keystrokes (keybind and chat actions) go to the focused window, but pure SDK broadcasts use `SendNotifyMessage(HWND_BROADCAST, …)` and arrive regardless of focus — those fail only on an integrity-level mismatch, which is what the elevation probe above covers.

Action code where focusing IS the pressed action calls `focusIRacingNow()` instead (#926) — the View Adjustment **Mouse to Sim** mode is the only consumer today. It shares the same result handling and logging but skips ALL THREE gates: the mode (it focuses even under `never`), the `isSettingsStoreReady()` startup gate and the post-timeout cooldown all exist to keep the *implicit* before-every-action focus from surprising anyone or stalling the plugin, and there is nothing implicit about pressing that key. Pointer placement is a separate service (`initMousePointer` / `movePointerToSim`), never gated on the focus setting for the same reason.
