...
**The Race Engineer voice migrates with no marker, because it can tell done from not-done (#1144).** Since #1144 a voice's identity outside its pack is `<pack id>::<voice id>`, and `raceEngineerVoice` stores that composite (`DEFAULT_RACE_ENGINEER_VOICE` is `default::default`). An earlier build stored the bare voice id, and `migrateRaceEngineerVoiceId(availableVoices, logger)` in `global-settings-migrations.ts` rewrites it to the composite it meant through `@iracedeck/callout-script`'s `qualifyVoiceId`: `default::<id>` when the managed pack provides that voice, else the alphabetically first pack id that provides it, else unchanged. Managed first, then alphabetical, is exactly the order that decided which pack's voice played before 3.3.0, so the voice a user was hearing is the voice they keep, a sideloaded one included. It gates on `isSettingsStoreReady()`, writes only when qualification changes the value, and never touches a composite or an unresolvable one — so it is idempotent, which is what lets it go without a marker and lets the plugins call it after every voice-pack scan and on every settings arrival, ahead of the missing-script re-assert. A bare value whose pack arrives later (a first start still downloading `default`) is therefore qualified when it does. `resolveActiveRaceEngineerVoice` applies the same rule at read time, so a hand-edited file, or a settings window from an older build sending a bare id, resolves before the write lands. Never widen it into a fallback: rewriting a value no pack provides would destroy the user's choice on a scan that merely failed to read a pack — the same rule `ird-voice-select` keeps (`@.claude/rules/stream-deck-actions.md`).
