...
const GlobalSettingsSchema = z.object({
  ...
  focusIRacingWindow: z.union([z.boolean(), z.string()])
    .transform(parseFocusIRacingMode) // "always" | "required" | "never"; true → always, false → never (#977)
    .default("always")
    .catch("always"),
...
**What the migration deadline is measured FROM (#1056).** `MIGRATION_TIMEOUT_MS` is armed when the read is issued, and then extended **exactly once** — by whichever comes first: the host connection becoming usable (`adapter.onHostReady?.(...)`, an **optional** `IDeckPlatformAdapter` member), or the budget expiring while a subscribed `onHostReady` still hasn't fired. So the budget is either measured from a point where the question could actually be asked, or bounded at twice it — never more, and the two paths share one flag so they cannot compound. The expiry-side grace is what covers a handshake slower than the whole budget, which the connect path cannot: by the time it fires there is no deadline left to move. Only the two WebSocket adapters implement it: they drop a frame written before their socket opens, so deck-core's read is issued into a closed socket and the client's connect-time reissue is what actually asks. Without the re-arm the connect latency came straight out of the budget — one Mirabox log showed 7.18 s between settings init and `Connected to VSD Craft`, against a 10 s deadline (one observation in one ordering, on a session that had a file and therefore armed no deadline at all; it is not a distribution and must not be treated as one). Elgato declares no `onHostReady`, because its SDK awaits the connection inside its own `send` (`@elgato/streamdeck` `dist/plugin/connection.js`) — so nothing is stubbed there, and absence means "the deadline already armed is honest". **The deadline is extended, never relocated, and that is a requirement rather than an implementation detail:** arming it only on connect would mean a host that never connects never arms, never times out and never gives up — no `_migrationPending` countdown, no ceiling, no `_migrationAbandoned` — trading a slow migration for the #1041 failure it exists to prevent. The extension is spent once per start, so neither a flapping socket nor a long handshake can widen the window without bound. Only an adapter that declares `onHostReady` gets either path — Elgato's read is queued by its own SDK until it can be sent, so its budget was never spent waiting for a connection and it gives up on the original schedule. **Do not try to grant the grace only to a host that is genuinely still connecting:** measured on both clients, a host mid-handshake and one that accepts TCP and never upgrades both read `CONNECTING` at the deadline, and a host that is genuinely absent never reaches the deadline at all, because the socket is refused and both clients end the process on close. The cost is accepted deliberately: a host connecting at ~7 s means the store can be un-ready for roughly twice the budget, and everything gated on `isSettingsStoreReady()` is degraded for that whole window — key bindings read as unset, `focusIRacingIfEnabled()` and `focusIRacingBeforeInput()` both skip the focus, and the plugin's store-ready block has not yet run, so a Property Inspector opened in that window bootstraps with no `_settingsChannel`, falls back to the deck host's copy, and shows edits as saved that never reach the file. Accepted as slow-and-correct in place of fast-and-wrong: the alternative is a healthy host being pushed into the give-up countdown. Weigh that full list, not just the bindings, before widening the window further. As ever, the fix is not a larger `MIGRATION_TIMEOUT_MS` — #1041 and #1047 both rejected that by name.
...
**`isSettingsStoreReady()`** reports whether the cache reflects the store — loaded, migrated, or fresh. It replaced `hasReceivedHostSettings()`, which is **removed**. Before it flips, the cache is pure schema defaults with no passthrough keys, so anything that must not act on defaults gates on it: `focusIRacingIfEnabled()` and `focusIRacingBeforeInput()`, the one-shot key migrations in `global-settings-migrations.ts`, and each plugin's `startupDefaultsApplied` block. Any consumer deciding on the ABSENCE of a key must wait for it. `isGlobalSettingsInitialized()` is the weaker, different thing (`initGlobalSettings` has been called) — never use it as this gate.

**`whenSettingsStoreSettled()`** (#1034 stage 3) is the third thing, and it is a WAIT rather than a gate: one promise that resolves once the load has SETTLED — ready by any path, the fail-closed give-up after `LOAD_ATTEMPTS` unreadable-file retries, or a throw while applying the loaded settings before the store was ready (the migration read throwing synchronously, say — nothing after that would ever settle the run, and a waiter parked there would be a launch step that never ensures, never retries and ignores every poke). `isSettingsStoreReady()` deliberately stays false on both failure paths, so a consumer that must not act on defaults still gates on ready; a consumer that merely wants the cache to hold whatever the file had, and is content to proceed on defaults when there is no file to be had, awaits this. The voice-pack launch step is the one caller: it must read the `_devBaseUrl` catalog override before it fetches, and a plugin whose settings file cannot be read must still end up with a voice. It never rejects, and it resolves immediately once either outcome has already happened. The signal is **re-armed per run** by `initGlobalSettings` (and by `_resetGlobalSettings`), so ask for it AFTER init — a promise taken before that call belongs to the signal init replaces and never resolves. That is why the launch step takes its `settled` dep as a THUNK.
...
**The Race Engineer voice migrates with no marker, because it can tell done from not-done (#1144).** Since #1144 a voice's identity outside its pack is `<pack id>::<voice id>`, and `raceEngineerVoice` stores that composite (`DEFAULT_RACE_ENGINEER_VOICE` is `default::default`). An earlier build stored the bare voice id, and `migrateRaceEngineerVoiceId(availableVoices, logger)` in `global-settings-migrations.ts` rewrites it to the composite it meant through `@iracedeck/callout-script`'s `qualifyVoiceId`, in this order: an empty or composite value is unchanged; a bare value that is itself an available voice is unchanged (a real bare voice — the scenario harness's source-tree `default`, or a voice a plugin might one day bundle again — not a pre-#1144 value to reinterpret); then `default::<id>` when the managed pack provides that voice; then the alphabetically first pack id that provides it; otherwise unchanged. Managed first, then alphabetical, is the order that decided which pack's voice played before 3.3.0 for every lowercase pack folder — which is every folder the installer writes — so the voice a user was hearing is the voice they keep, a sideloaded one included. It is not the same order in every case: that scanner sorted folder names case-sensitively, so a hand-made folder with capitals (`Zeta/`) sorted before `alpha/` there and after it here; accepted while no third-party pack exists. It gates on `isSettingsStoreReady()` AND on the managed pack being in the scan (some `default::…` id available): before `default` has arrived the alphabetical half would decide alone, and a leftover sideload declaring a voice called `default` would have its composite written down for good, a voice the user never heard. It writes only when qualification changes the value and never touches a composite or an unresolvable one — so it is idempotent, which is what lets it go without a marker and lets the plugins call it after every voice-pack scan and on every settings arrival, each time right before the missing-script re-assert. A bare value is therefore persisted by the scan that brings the managed pack (a first start still downloading `default` writes nothing until then). `resolveActiveRaceEngineerVoice` applies the same rule at read time and covers the gap: the engineer speaks meanwhile, and a hand-edited file, or a settings window from an older build sending a bare id, resolves before any write lands. Never widen it into a fallback: rewriting a value no pack provides would destroy the user's choice on a scan that merely failed to read a pack — the same rule `ird-voice-select` keeps (`@.claude/rules/stream-deck-actions.md`).

**Passthrough keys the store introduces:** `_settingsStorePath` (the resolved file path, shown on Diagnostics) — plugin-written, not a user setting. `_firstRunVersion` (#1061) records the plugin version that resolved the Getting Started decision; it holds a VERSION rather than `true` for the #1041→#1047 reason, is written on every resolution including "existing install, show nothing", and must **never** be run-scoped — that list is stripped at three boundaries and the page would reappear on every start. `_migrationPending` (the countdown) and `_migrationAbandoned` (#1041, durable) are the migration's own two markers; both are plugin-written and both **must persist**, and the second one is the only thing standing between an abandoned install and its host copy being overwritten — so it must never be routed into a store a user or a maintenance step is told is safe to delete. `_settingsChannel` (`{ port, token }`) is deliberately NOT stored in the file: it is per-process (a new port and token every start) and nothing reads it from the store, so the publisher removes a stale copy an older build left there and the channel travels only inside the deck-host mirror (which is what a PI's bootstrap read returns). `_voiceLabels` (#1034) is the one voice-pack key that IS persisted, keyed like `_raceEngineerVoices` by composite voice id (`<pack id>::<voice id>`, #1144): it is published in the same `updateGlobalSettings` call as `_raceEngineerVoices` and must share its lifetime, since a name map outliving its voice list is exactly the drift the single write exists to prevent. `_warnings` is likewise never in the file, by a different mechanism and for a different reason — it is enrolled as a **run-scoped key** (#1014), see the section on it below. Both keys reach Property Inspectors again — `_settingsChannel` through the mirror, everything else through the loopback pushes — so a PI's Diagnostics row shows `_settingsStorePath` and `_warnings` / `_audioDeviceList` / `_deckDevices` are live there as well (once the PI has bootstrapped onto the loopback channel; a fallen-back PI sees only the once-per-start host mirror, not live pushes).
...
Reference producers, all in deck-core and all wired in every plugin's `plugin.ts`, and all following the same two-module split — a **pure** evaluator returning `PiWarning | null`, plus a thin adapter that is the only part touching the warning store:
...
- **Missing callout script** (#1064) — `createVoiceScriptWarningReporter({ set: setWarning, clear: clearWarning })` wrapping `evaluateVoiceScriptWarning({ activeVoice, scriptedVoices, labels })`, id `voice-script-missing`. Raised while the ACTIVE Race Engineer voice (the output of `resolveActiveRaceEngineerVoice`) has no parsed script in `voicePackService.scripts()` — a clips-only pack, or one whose `callouts.json` the scanner refused. Silence is correct for that pack (a pack is never punished for what it does not say) and wrong for the user who picked the voice and hears nothing, so the condition reaches the surface the user is looking at, naming the voice and both remedies (reinstall the pack, or pick another voice). It names the voice as the dropdown does — its `_voiceLabels` name, else the title-cased voice half of its composite id — never as the raw `<pack id>::<voice id>` (#1144). The banner says every callout *that comes from the script* is skipped in that voice. Since #1065 that is every callout — the whole catalog is scripted, so a scriptless voice IS a silent engineer — and the phrasing was chosen to stay true either way: it was written while only the flags family was scripted (#1064) and needed no change when the rest followed. The plugins re-evaluate it after every voice-pack scan and on every global-settings change, which is where the active voice changes — each time straight after `migrateRaceEngineerVoiceId`, so it judges the qualified value.

### `_warnings` is run-scoped — never persisted (#1014)

`_warnings` is enrolled in `RUN_SCOPED_SETTING_KEYS` (`deck-core/src/run-scoped-settings.ts`) — as is `_voicePacks` since #1034, the last voice-pack scan (`{ packs, problems }`), for the same reason stated the other way round: it describes what is on disk during THIS run, so persisting it would let a pack the user deleted reappear in the settings window after a restart. `_voicePackStatus` joins them in #1100 — the catalog's last answer plus any install in flight or failed — on the same test: a download that was in flight when the plugin stopped is not in flight any more, and persisting it would put a frozen progress bar, or an error nobody ever saw, in front of a user on a run where neither is true. Every boundary an older value could cross back in through strips both:
...
- **Every producer must re-assert its state within the run** — that is the price of the guarantee. A producer that only speaks when something *changes* would go silent about a condition that is still true. Six of the seven today comply, and the seventh is a deliberate exception explained at the end: the settings-window reporter reports at `ensureStarted()` on every start, `validateSetupWarningPatterns` runs on the first settings arrival and on every change after it, the elevation probe runs on each iRacing connection (so after a restart its banner is correctly absent until there is a live sim to compare integrity levels with), the voice-pack service publishes `_voicePacks` after every scan — one at startup, one per Rescan press — plus on every Property Inspector appearance, the installer republishes `_voicePackStatus` on every phase change, so a run that begins with no catalog answer and no install in flight says exactly that, and the missing-script reporter (#1064) re-evaluates `voice-script-missing` after every voice-pack scan and on every global-settings change — on a fresh run it is the FIRST SETTINGS ARRIVAL after the startup scan that raises it, not the scan itself (the startup scan runs before `initGlobalSettings`, so `onPacksChanged` returns early behind `isGlobalSettingsInitialized()` and the settings listener re-asserts once the cache holds the active voice), so a scriptless voice selected last time is named again the moment this run knows both the packs and the choice. The exception is the voice-pack REMOVAL banner (#1100), which posts to `_warnings` when a removal fails and clears on the next success. It is event-driven rather than state-driven and owes no re-assertion, because the condition it reports does not outlive the run: a removal the user attempted last time the plugin ran is not a fact about this one, and re-announcing it on a fresh start would be exactly the resurrected-banner behaviour run-scoping exists to prevent. A producer may only claim this exemption when its condition genuinely expires with the process — the elevation mismatch and the dead settings server do not, which is why they re-assert. Wire a new producer the same way, and add its re-assertion to `plugin.ts` in all three plugins.
