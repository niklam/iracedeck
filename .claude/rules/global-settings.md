...
**The changelog never opens while iRacing is running** (issue #870 — a deck-host auto-update restarts the plugin mid-session, and the page must not pop over a live race). `runVersionCheck` takes an optional `isSimRunning` delegate consulted only when an `open` is actually due: when it reports true the open **defers** — nothing persisted, the version stays pending — with `track-silently` outcomes unaffected (nothing opens anyway). Plugins pass `isIRacingActive` from `app-monitor.ts` (running-flag OR live SDK connection — the two signals cover each other's gaps, e.g. hosts whose app-monitoring events are unproven) and re-run the check from the app monitor's `onIRacingTerminated` hook, which notifies **after** the running flag and SDK connection are already down (`setReconnectEnabled(false)` actively disconnects), so a deferred page opens right as the sim exits. The startup check itself is delayed by `VERSION_CHECK_STARTUP_GRACE_MS` (15 s, exported from `version-check.ts`) because on a mid-session restart the launch event / SDK connection race the first settings arrival — an immediate check could read "not running" and open over the session. Each plugin wraps all of this in one `runChangelogVersionCheck()` that re-reads the **live** settings cache per call (it runs at two different times), guarded on `startupDefaultsApplied` so a terminate before the first settings arrival is a no-op; `runVersionCheck` is naturally idempotent across the calls (persisted version → `skip`). User-facing behavior is documented on the website: `docs/features/whats-new-page.md`.
