...
This renders the PI's whole bottom section: a "Key Bindings" header with the "Open iRaceDeck Settings" button, and the action's bindings in a collapsible `Related Key Bindings` accordion. Omit `keyBindings` for an action that has none — the section then says so rather than showing an empty accordion.

Since #1003 this is the **only** plugin-global surface left in an action PI. Every other global setting lives in the dedicated settings window; do not add one to a PI.
