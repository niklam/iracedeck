...
**Phase 1 never writes the deck host.** Each plugin's startup-defaults block publishes `_settingsChannel: { port, token }` — the loopback settings server's address, once `settingsWindow.ensureStarted()` resolves — into the store, and **only** into the store. The host's copy is read once for migration and never written again, so it stays intact as the downgrade safety net.

**The phase-2 bootstrap write must be a guarded mirror.** The PI bridge needs `_settingsChannel` in the host copy to find the loopback socket before it can talk to the plugin, so phase 2 adds one write per start — and it cannot be a partial. Every deck host's `setGlobalSettings` **REPLACES** the whole stored object (that is the #896 history, and `scenario-harness/src/mock-platform-adapter.test.ts` asserts it), so a bare `{ _settingsChannel }` would truncate the host copy to a single key: PIs blank, downgrade net gone. It must send `{ ...getGlobalSettings(), _settingsChannel }`, and it must **skip the write entirely when the store became ready via the migration timeout** — the host never answered, so the cache is schema defaults and writing it would clobber a copy the plugin never got to read.
