...
## Installing / switching — the "Stream Deck Profiles" accordion

Bundled profiles do **not** reliably auto-install on plugin updates. The install path iRaceDeck relies on is: **switching to a profile prompts the Stream Deck app to install it** when it isn't installed yet. So the settings window's Profiles pane shows a **"Stream Deck Profiles" accordion** (partial `global-stream-deck-profiles.ejs`) with one **button per bundled profile template** — the rows are the unique `displayName`s from the generated `profiles.json`, so device variants of one template collapse into a single suffix-free row (#753) and a new template gets a row automatically once its files are in the manifest and `pnpm generate:action-profiles` has run.

Until #1003 this accordion also appeared in every action PI (via the `global-common-settings.ejs` assembler, now deleted). The window is the only place it lives now — which suits it, since the window's device picker states the target deck explicitly instead of inferring it from whichever PI happens to be open.

Flow: `<ird-profile-switch profile="<display name>">` (in `@iracedeck/pi-components`) → on click sends `sendToPlugin` `{ event: "switchToProfile", profile }` → `ElgatoPlatformAdapter` (which wires `streamDeck.ui.onSendToPlugin` in its constructor) resolves the pressing device's manifest name via `deviceProfileName(profile, ev.action.device.type)` (#753) and routes it through `requestProfileSwitch`, targeting the device whose PI is open — both steps live in `ElgatoPlatformAdapter.switchToBundledProfile(deviceId, profile, page?, deviceType?)`. The settings window (#992) has no implicit device: its Profiles pane adds `ird-deck-device-select` (fed by the `_deckDevices` passthrough global the Elgato plugin publishes on device connect/disconnect and on open) and its buttons carry `device-from`, so the payload names the `deviceId`; the plugin's `createSettingsWindowCommandHandler` requires it and calls the same `switchToBundledProfile` (type looked up via `streamDeck.devices.getDeviceById`).
