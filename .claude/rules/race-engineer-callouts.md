...
- **Freeze finished cars, quiet retired cars, tune reaction frequency** — issue #603. Demonstrates **a stateful wrapper over a pure SDK helper** plus **classifying a callout by the cause of the position change**, all without a new bus event or opt-in. (1) **Finished-car freeze.** `calculateRacePositions` (pure, `@iracedeck/iracing-sdk`) ranks only `CarIdxLapDistPct >= 0` cars, so a finisher leaving the world shifts everyone behind up a rank and surfaced a string of phantom "Nice pass" gains for a mid-pack driver. `updateFinishedCars` (`diff/race-finish.ts`) tracks, per car, which crossed S/F under the global checkered — leader frozen on the checkered rising edge (its increment may not be observable on the same tick, the same counter/field race `diffLaps` works around), everyone else on their `CarIdxLapCompleted` increment, plus an additive, gated `CarIdxSessionFlags & Checkered` corroborator (`USE_PER_CAR_CHECKERED`, the bit's per-car semantics are undocumented so it can only ever ADD finishers while the global checkered is up). `calculateFrozenRacePositions` re-injects a frozen car at its finishing-order score once it leaves the world; the wrapper lives in `sim-events-iracing` (it needs `TranslatorState`) while the SDK function stays pure (threading translator state into `iracing-sdk` would invert layering). Both `diffOvertakes` (detection) and `getLivePosition` (the spoken overall readout) consume it. (2) **Retirement classification.** With finishers frozen, the only remaining cause of a position-shift gain is a non-finished car ahead leaving — a retirement / DNF / disconnect. `diffOvertakes` flags it via `isRetirementGain` (a car ranked ahead in the previous tick's RAW active positions, `state.lastActivePositions`, is inactive now and NOT in `finishedCarScores`), latched across the hold window so a blended retire-plus-pass is caught, emitted as the new optional `overtake.completed.fromRetirement`. The reaction `where:` returns false on `fromRetirement` (readout only, no "Nice pass"); single-class only — multi-class detects on `PlayerCarClassPosition`, which iRacing recomputes correctly when a car leaves. (3) **Sparser reactions + podium.** `shouldReactToOvertake(effectivePosition)` replaces the old 20 s `tryClaimReaction` cooldown: P1/P2/P3 always react (each with a dedicated line — P2/P3 added alongside the existing P1 leader line, single + "…in class" variants), everything else rolls `REACTION_CHANCE` (~1/3, via an injectable `reactionRandom` test seam). Podium gains suppress the follow-up position readout (their line states the position). (4) **Bare/full intro.** `shouldSpeakIntro` drops the "We're currently" intro for a bare "P[n]" when it was spoken within `INTRO_COOLDOWN_MS` (30 s) AND the move is ≤1 position; a >1 jump always restores the full intro. Tracked in module state (`lastIntroAt` / `lastSpokenPosition`) and scoped to the overtake position readout (the shared `readoutSequence`); race-status / position-change keep their own always-full intro, since the existing 20 s cross-trigger position cooldown already prevents their double-announce. Reach for this freeze-wrapper pattern when a pure helper's output needs cross-tick memory only some consumers want; and for the cause-classification pattern when the *same* position change should produce different audio depending on whether it was earned (a pass) or inherited (a retirement).
