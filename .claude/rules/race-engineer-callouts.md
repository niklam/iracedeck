...
- **Pit-box count-in** — issue #600. Demonstrates **a callout driven by a derived continuous distance rather than a sim enum/state diff**. The translator resolves the player's box from `SessionInfo.DriverInfo.DriverPitTrkPct` (`resolvePitBoxTrkPct`) and `diffPitBoxCountdown` computes the forward gap from `LapDistPct` to the box, folded around the lap, times the cached `state.trackLengthMeters` (reused from #574). The matching mark is the **band** the remaining distance falls into — the smallest threshold (120/100/80/60/40/20 m) still ≥ the gap, with a `1e-6` epsilon so a `pct × length` value a hair above an exact boundary still lands in the intended band — so a normal approach fires `five`→`pit-now` once each, entering pit road already within range fires only the marks still ahead (no burst of skipped numbers), and once the box is passed the gap folds to ~a full lap so no band matches and the count stops. **No first-tick seed / `initialized` flag** — there's no connect-time transition to suppress; instead `state.pitBoxMarksSpoken: Set<PitBoxMark>` dedups within a visit and is **cleared whenever the car is off pit road**, so a second stop counts down again. Gated on `OnPitRoad` and **suppressed in the stall** (`PlayerCarInPitStall`) so an arrived/connecting-in-box car gets no spurious "pit now". The audio side is **six per-mark scenarios → one `count-in` opt-in** (the track-conditions multi-scenario/single-subject shape), `family: "pit-box"` for preempt, and **no radio-open/close frame** (terse countdown). New bus event `pitBox.countdown` with a `{ mark: PitBoxMark }` payload; the box position is known up front so the count-in works on the **first** stop of a session — no learn-once needed. Reach for this pattern when the trigger is a continuous measurement (distance, time-to-X) rather than a discrete state change: band-select on the live value, dedup per-episode in a Set keyed to the activity window, and reset on the window's exit edge.
