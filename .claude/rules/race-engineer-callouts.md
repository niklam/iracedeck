...
| **Pack linter (#1066)** | `packages/audio-scenarios/src/reference/lint-pack.ts` (`lintPack`, `formatLintReport`) behind `scripts/lint-pack.mjs` + `scripts/lib/lint-pack-run.mjs` — `pnpm lint:pack <packDir>` from the repo root after `pnpm build`; exit 0 clean, 1 on any problem, 2 when it could not run. Says loudly what the plugin would only skip quietly: grammar problems, every compile skip the pack did not mean with its reason, failed frames and fragments, clips shipped that nothing references, references to clips not shipped, a `callouts.json` over the scanner's 1 MiB cap (reported unread, as the plugin refuses it), and a `<voice>: N of M callouts scripted` summary per voice. The clip groups plugin code plays by path outside any script — `names/` and `toggle/` — are `PLUGIN_PLAYED_GROUPS` in `lib/lint-pack-run.mjs` (the runner's knowledge, passed in as `pluginPlayedGroups`; the pure linter defaults to none) and are never orphans. No per-pack allowlist — the bundled voice's own exceptions (`DELIBERATELY_UNSPOKEN`, `VAR_DRIVEN_BASES`) stay in `script-coverage.test.ts` as facts about that voice — so a lint of the bundled tree is not expected to be clean |
...
Then regenerate the artifact and commit it with the config — and, once the package is built, the website's reference, which reads the bundled script and the contract you added (step 11 has the order):
