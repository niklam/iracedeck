...
| **Audio pools** | A migrated family's script addresses its clips directly — `pool:<group>/<base>` — and names a pool (`pools` in `callouts.json`, looked up first for the active voice) only where the name carries a decision; the families still in code keep theirs in `packages/audio-scenarios/src/catalog/pit-crew/pools.ts` — `POOL_REGISTRY` maps pool name → manifest `(group, base)` — until #1065 empties it. Either way members (`<base>-NN.mp3`) derive per-voice from the manifest at fire time (issue #664) |
...
- Export `<FAMILY>_CONTRACTS: readonly ScenarioContract[]` (migrated / new family) or `<FAMILY>_ALERTS: readonly Scenario[]` (un-migrated), plus `<FAMILY>_SCENARIO_IDS` and either `<FAMILY>_CLIP_SOURCES: readonly { group, base }[]` (scripted family — a **literal** list of the `(group, base)` sources its script addresses, since nothing derives it; the bundled voice must ship a clip for each and the bundled script must reference exactly that set) or `<FAMILY>_POOL_NAMES` (un-migrated — the registry names).
